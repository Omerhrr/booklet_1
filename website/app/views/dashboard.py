...
from app import api_request, generate_csrf_token, validate_csrf_token, login_required
...
def index():
    ...
    # Get recent payments
    payments_result, _ = api_request(
        "GET", f"/saas/payments?business_id={business_id}&limit=5"
    )
    ...
    logins_result, _ = api_request(
        "GET", f"/saas/software-logins?business_id={business_id}"
    )
...
def cancel_subscription():
    ...
    result, status_code = api_request(
        "POST", "/saas/subscription/cancel", data={"business_id": business_id}
    )

    if status_code == 200:
        flash(
            "Subscription cancelled. You can continue using Booklet until the end of your billing period.",
            "success",
        )
    else:
