...
from app import api_request, generate_csrf_token, validate_csrf_token, admin_required
...
def plan_edit(plan_id):
    ...
    if request.method == "POST":
        if not validate_csrf_token(request.form.get("csrf_token")):
            flash("Invalid request. Please try again.", "error")
            return redirect(url_for("admin.plan_edit", plan_id=plan_id))
...
def contact_reply(contact_id):
    ...
    result, status_code = api_request(
        "POST", f"/saas/contact/{contact_id}/reply", data=data
    )
...
def contact_status(contact_id):
    ...
    result, status_code = api_request(
        "PUT", f"/saas/contact/{contact_id}", data={"status": status}
    )
