...
from app import api_request, generate_csrf_token, validate_csrf_token
import uuid
...
def auto_login():
    ...
    result, status_code = api_request(
        "POST", "/saas/auth/verify-token", data={"token": token}
    )
...
def login():
    ...
    if request.method == "POST":
        ...
        result, status_code = api_request(
            "POST", "/saas/auth/login", data={"email": email, "password": password}
        )
...
def register():
    ...
    selected_plan = next(
        (p for p in plans if p.get("slug") == plan_slug), plans[0] if plans else None
    )
    ...
    if request.method == "POST":
        ...
        _registration_cache[reg_token] = {
            "email": request.form.get("email"),
            "password": request.form.get("password"),
            "name": request.form.get("name"),
            "plan_slug": request.form.get("plan_slug", plan_slug),
        }
        session["registration_token"] = reg_token
...
def setup_business():
    ...
    reg_token = session.get("registration_token")
    registration = _registration_cache.get(reg_token) if reg_token else None
    if not registration:
    ...
    if request.method == "POST":
        ...
        if status_code == 200:
            _registration_cache.pop(reg_token, None)
            ...
            flash(
                result.get("detail", "Failed to create account. Please try again."),
                "error",
            )
...
def forgot_password():
    ...
    if request.method == "POST":
        ...
        result, status_code = api_request(
            "POST", "/saas/auth/forgot-password", data={"email": email}
        )
