...
async function fetchVendors() {
  loading.value = true
  try {
    const { data } = await crmApi.listVendors()
    vendors.value = Array.isArray(data) ? data : data.items || data.vendors || []
  } catch (error) {
    console.error('Failed to fetch vendors:', error)
    toastStore.show('Failed to load vendors', 'error')
  } finally {
    loading.value = false
  }
}
...
function viewVendor(vendor) {
  router.push({ name: 'VendorDetail', params: { id: vendor.id } })
}

function editVendor(vendor) {
  router.push({ name: 'VendorEdit', params: { id: vendor.id } })
}
...
async function handleDelete() {
  ...
  deleting.value = true
  try {
    await crmApi.deleteVendor(selectedVendor.value.id)
...
async function toggleStatus(vendor) {
  try {
    await crmApi.toggleVendorStatus(vendor.id)
    vendor.status = vendor.status === 'active' ? 'inactive' : 'active'
    toastStore.show(`Vendor ${vendor.status === 'active' ? 'activated' : 'deactivated'}`)
  } catch (error) {
...
<template>
  <div>
    <PageHeader title="Vendors" :breadcrumbs="breadcrumbs">
      <template #actions>
        <router-link
          v-if="authStore.hasPermission('vendors:create')"
    ...
    <!-- Empty State -->
    <EmptyState
      v-if="!loading && vendors.length === 0"
      title="No vendors yet"
      message="Get started by adding your first vendor."
      :action-text="authStore.hasPermission('vendors:create') ? 'Add Vendor' : ''"
      action-route="/crm/vendors/new"
    />
