...
const breadcrumbs = computed(() => [
  { text: 'CRM', to: '/crm' },
  { text: 'Vendors', to: '/crm/vendors' },
  { text: vendor.value?.name || 'Vendor Details' },
])

async function fetchVendor() {
  loading.value = true
  try {
    const { data } = await crmApi.getVendor(route.params.id)
    vendor.value = data
    bills.value = data.bills || []
    payments.value = data.payments || []
  } catch (error) {
    console.error('Failed to fetch vendor:', error)
    toastStore.show('Failed to load vendor', 'error')
  } finally {
    loading.value = false
  }
}
...
function editVendor() {
  router.push({ name: 'VendorEdit', params: { id: route.params.id } })
}
...
async function toggleStatus() {
  ...
  try {
    await crmApi.toggleVendorStatus(vendor.value.id)
    vendor.value.status = vendor.value.status === 'active' ? 'inactive' : 'active'
    toastStore.show(`Vendor ${vendor.value.status === 'active' ? 'activated' : 'deactivated'}`)
  } catch (error) {
...
async function handleDelete() {
  deleting.value = true
  try {
    await crmApi.deleteVendor(route.params.id)
    toastStore.show('Vendor deleted successfully')
    router.push({ name: 'VendorList' })
  } catch (error) {
    console.error('Failed to delete vendor:', error)
    toastStore.show('Failed to delete vendor', 'error')
  } finally {
...
<template>
  <div>
    ...
    <template v-else-if="vendor">
      <PageHeader :title="vendor.name" :breadcrumbs="breadcrumbs">
        <template #actions>
          <div class="flex items-center gap-3">
            ...
            <button
              v-if="authStore.hasPermission('vendors:edit')"
              type="button"
              :class="[
            ...
            <button
              v-if="authStore.hasPermission('vendors:delete')"
              type="button"
      ...
      <div v-if="activeTab === 'info'" class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
        ...
        <dl class="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4">
          <div v-if="vendor.email">
            ...
            <dd class="text-sm font-medium text-gray-900 dark:text-white mt-0.5">{{ vendor.email }}</dd>
          </div>
          <div v-if="vendor.phone">
            <dt class="text-sm text-gray-500 dark:text-gray-400">Phone</dt>
            <dd class="text-sm font-medium text-gray-900 dark:text-white mt-0.5">{{ vendor.phone }}</dd>
          </div>
          <div v-if="vendor.address">
            <dt class="text-sm text-gray-500 dark:text-gray-400">Address</dt>
            <dd class="text-sm font-medium text-gray-900 dark:text-white mt-0.5">{{ vendor.address }}</dd>
          </div>
          <div v-if="vendor.city">
            <dt class="text-sm text-gray-500 dark:text-gray-400">City</dt>
            <dd class="text-sm font-medium text-gray-900 dark:text-white mt-0.5">{{ vendor.city }}</dd>
          </div>
          <div v-if="vendor.state">
            <dt class="text-sm text-gray-500 dark:text-gray-400">State</dt>
            <dd class="text-sm font-medium text-gray-900 dark:text-white mt-0.5">{{ vendor.state }}</dd>
          </div>
          <div v-if="vendor.country">
            <dt class="text-sm text-gray-500 dark:text-gray-400">Country</dt>
            <dd class="text-sm font-medium text-gray-900 dark:text-white mt-0.5">{{ vendor.country }}</dd>
          </div>
          <div v-if="vendor.tax_id">
            <dt class="text-sm text-gray-500 dark:text-gray-400">Tax ID</dt>
            <dd class="text-sm font-medium text-gray-900 dark:text-white mt-0.5">{{ vendor.tax_id }}</dd>
          </div>
          <div v-if="vendor.created_at">
            <dt class="text-sm text-gray-500 dark:text-gray-400">Created</dt>
            <dd class="text-sm font-medium text-gray-900 dark:text-white mt-0.5">{{ formatDate(vendor.created_at) }}</dd>
          </div>
          <div v-if="vendor.notes" class="sm:col-span-2">
            <dt class="text-sm text-gray-500 dark:text-gray-400">Notes</dt>
            <dd class="text-sm font-medium text-gray-900 dark:text-white mt-0.5 whitespace-pre-wrap">{{ vendor.notes }}</dd>
          </div>
        </dl>
      </div>

      <!-- Bills Tab -->
      <div v-if="activeTab === 'bills'" class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div v-if="bills.length === 0" class="flex flex-col items-center justify-center py-12 text-center">
          <p class="text-sm text-gray-500 dark:text-gray-400">No bills found for this vendor.</p>
        </div>
        <div v-else class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead class="bg-gray-50 dark:bg-gray-800">
              <tr>
                <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Bill #</th>
            ...
            <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
              ...
              >
                <td class="px-6 py-4 text-sm font-medium text-blue-600 dark:text-blue-400">{{ bill.bill_number }}</td>
                <td class="px-6 py-4 text-sm text-gray-700 dark:text-gray-300">{{ formatDate(bill.date) }}</td>
                <td class="px-6 py-4 text-sm font-medium text-gray-900 dark:text-white">{{ formatBalance(bill.amount) }}</td>
                <td class="px-6 py-4"><StatusBadge :status="bill.status" /></td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <div v-if="activeTab === 'payments'" class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div v-if="payments.length === 0" class="flex flex-col items-center justify-center py-12 text-center">
          <p class="text-sm text-gray-500 dark:text-gray-400">No payments found for this vendor.</p>
        </div>
        <div v-else class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead class="bg-gray-50 dark:bg-gray-800">
    ...
    <ConfirmDialog
      ...
      title="Delete Vendor"
      :message="`Are you sure you want to delete '${vendor?.name}'? This action cannot be undone and will remove all associated data.`"
      confirm-text="Delete"
      type="danger"
