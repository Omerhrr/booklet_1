...
const breadcrumbs = computed(() => [
  ...
  { text: form.name || 'Vendor', to: { name: 'VendorDetail', params: { id: route.params.id } } },
  { text: 'Edit' },
])
...
async function handleSubmit() {
  ...
  loading.value = true
  try {
    await crmApi.updateVendor(route.params.id, form)
    toastStore.show('Vendor updated successfully')
    router.push({ name: 'VendorDetail', params: { id: route.params.id } })
  } catch (error) {
    console.error('Failed to update vendor:', error)
    const message = error.response?.data?.detail || error.response?.data?.message || 'Failed to update vendor'
...
<template>
  <div>
    ...
    <template v-else>
      <PageHeader title="Edit Vendor" :breadcrumbs="breadcrumbs" />
