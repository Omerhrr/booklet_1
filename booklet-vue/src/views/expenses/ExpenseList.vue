...
import { listExpenses, getExpenseCategories, deleteExpense } from '@/api/expenses'
...
async function fetchExpenses() {
  ...
  try {
    ...
    expenses.value = Array.isArray(data) ? data : data.items || data.expenses || []
  } catch (error) {
...
async function fetchBankAccounts() {
  try {
    ...
    bankAccounts.value = (Array.isArray(data) ? data : data.accounts || []).map(a => ({
      value: a.id,
...
function editExpense(expense) {
  router.push({ name: 'ExpenseEdit', params: { id: expense.id } })
}
  ...
  <div class="space-y-6">
    ...
    >
      <template #actions>
        <div class="flex items-center gap-2">
          ...
          <button
            v-if="auth.hasPermission('expenses:create')"
            type="button"
            ...
            @click="router.push({ name: 'ExpenseCreate' })"
          >
    ...
    >
      ...
      <template #cell-reference="{ row }">
        ...
        >
          {{ row.reference || `EXP-${String(row.id).padStart(4, '0')}` }}
        </button>
      </template>
      ...
      <template #cell-amount="{ row }">
        <span class="text-sm font-medium text-gray-900 dark:text-white">{{ formatCurrency(row.amount, auth.branchCurrency) }}</span>
      </template>
    ...
    <ConfirmDialog
      ...
      :message="`Are you sure you want to delete expense '${deleteTarget?.reference || deleteTarget?.id}'? This action cannot be undone.`"
      confirm-text="Delete"
      cancel-text="Cancel"
      type="danger"
