...
import { getOtherIncome, updateOtherIncome, getOtherIncomeCategories } from '@/api/otherIncomes'
...
async function fetchIncome() {
  ...
  try {
    const [incomeRes, categoriesRes, accountsRes] = await Promise.all([
      getOtherIncome(route.params.id),
      getOtherIncomeCategories(),
      getPaymentAccounts(),
    ])

    const data = incomeRes.data
    form.value = {
      date: data.date || '',
...
async function handleSubmit() {
  ...
  try {
    ...
    await updateOtherIncome(route.params.id, payload)
    toast.show('Other income updated successfully', 'success')
    router.push({ name: 'OtherIncomeDetail', params: { id: route.params.id } })
  } catch (error) {
    console.error('Failed to update other income:', error)
    const message = error.response?.data?.message || error.response?.data?.detail || 'Failed to update other income'
...
<template>
  <div class="space-y-6">
    ...
    <LoadingSpinner v-if="fetchLoading" text="Loading other income..." />

    <form v-else class="max-w-3xl mx-auto space-y-6" @submit.prevent="handleSubmit">
      <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
        <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Income Details</h2>
      ...
      <div class="flex flex-col-reverse sm:flex-row items-center justify-end gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
        ...
        >
          ...
          {{ loading ? 'Saving...' : 'Update Other Income' }}
        </button>
      </div>
    </form>
  </div>
