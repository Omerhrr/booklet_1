...
import { createOtherIncome, getOtherIncomeCategories, getNextOtherIncomeNumber } from '@/api/otherIncomes'
...
async function handleSubmit() {
  ...
  try {
    ...
    toast.show('Other income created successfully', 'success')
    router.push({ name: 'OtherIncomeList' })
  } catch (error) {
    console.error('Failed to create other income:', error)
    const message = error.response?.data?.message || error.response?.data?.detail || 'Failed to create other income'
    ...
    <form v-else class="max-w-3xl mx-auto space-y-6" @submit.prevent="handleSubmit">
      ...
      <div class="flex flex-col-reverse sm:flex-row items-center justify-end gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
        ...
        >
          ...
          {{ submitting ? 'Creating...' : 'Create Other Income' }}
        </button>
      </div>
    </form>
  </div>
