...
import { getInvoice, createCreditNote } from '@/api/sales'
...
async function fetchInvoice() {
  ...
  try {
    ...
    invoice.value = data
    // Pre-fill line items from the original invoice
...
async function handleSubmit() {
  ...
  try {
    ...
    toast.show('Credit note created successfully', 'success')
    router.push({ name: 'CreditNoteList' })
  } catch (error) {
    console.error('Failed to create credit note:', error)
    const message = error.response?.data?.message || error.response?.data?.detail || 'Failed to create credit note'
...
function handleCancel() {
  router.push({ name: 'InvoiceDetail', params: { id: route.params.id } })
}
  ...
  <div class="space-y-6">
    <PageHeader
      ...
      :breadcrumbs="[
        ...
        { text: invoice?.invoice_number || 'Invoice', to: { name: 'InvoiceDetail', params: { id: route.params.id } } },
        { text: 'Credit Note' }
      ]"
    />

    <LoadingSpinner v-if="loading" text="Loading invoice data..." />

    <form v-else class="space-y-6" @submit.prevent="handleSubmit">
      <!-- Invoice Reference -->
      <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
        <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Invoice Reference</h2>
        <div class="flex items-center gap-4">
          ...
          <div class="flex items-center gap-2 px-4 py-2.5 bg-gray-50 dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700">
            ...
            <span class="text-sm font-medium text-gray-900 dark:text-white">
              {{ formatCurrency(invoice?.total, auth.branchCurrency) }}
            </span>
          </div>
        </div>
      </div>
      ...
      <!-- Line Items -->
      <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
        <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Credit Note Items</h2>
        <p class="mb-4 text-sm text-gray-500 dark:text-gray-400">Edit the items to include in this credit note. Adjust quantities as needed.</p>
