...
import { getBill, recordPayment } from '@/api/purchases'
import { createDebitNote } from '@/api/purchases'
...
const balanceDue = computed(() => {
  ...
  const total = Number(bill.value.total) || 0
  const paymentsTotal = (bill.value.payments || []).reduce((sum, p) => sum + (Number(p.amount) || 0), 0)
...
function handleCreateDebitNote() {
  router.push({ name: 'DebitNoteCreate', params: { id: route.params.id } })
}
...
<template>
  <div class="space-y-6">
    ...
    >
      <template #actions>
        <div class="flex items-center gap-2">
          ...
          <button
            v-if="bill && bill.status !== 'cancelled' && bill.status !== 'paid'"
            type="button"
    ...
    <LoadingSpinner v-if="loading" text="Loading bill..." />
    ...
    <template v-else-if="bill">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
        <div class="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
          ...
          <div class="flex items-center gap-2">
            <button
              v-if="bill.status !== 'cancelled' && bill.status !== 'written_off'"
              type="button"
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
        ...
        <div class="space-y-1">
          ...
          <p v-if="bill.vendor?.email" class="text-sm text-gray-500 dark:text-gray-400">{{ bill.vendor.email }}</p>
          <p v-if="bill.vendor?.phone" class="text-sm text-gray-500 dark:text-gray-400">{{ bill.vendor.phone }}</p>
          <p v-if="bill.vendor?.address" class="text-sm text-gray-500 dark:text-gray-400">{{ bill.vendor.address }}</p>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div class="xl:col-span-2 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
          ...
          <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                <tr
                  v-for="(item, index) in (bill.line_items || [])"
                  :key="index"
          ...
          <!-- Notes & Terms -->
          <div v-if="bill.notes || bill.terms" class="mt-6 pt-4 border-t border-gray-200 dark:border-gray-700 space-y-3">
            <div v-if="bill.notes">
              <h4 class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Notes</h4>
              <p class="mt-1 text-sm text-gray-700 dark:text-gray-300 whitespace-pre-wrap">{{ bill.notes }}</p>
            </div>
            <div v-if="bill.terms">
              <h4 class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Terms</h4>
              <p class="mt-1 text-sm text-gray-700 dark:text-gray-300 whitespace-pre-wrap">{{ bill.terms }}</p>
            </div>
          </div>
        </div>
          ...
          <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
            ...
            <div class="space-y-3">
              <div class="flex items-center justify-between">
                ...
                <span class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ formatCurrency(bill.subtotal, auth.branchCurrency) }}</span>
              </div>
              <div class="flex items-center justify-between">
                <span class="text-sm text-gray-500 dark:text-gray-400">Tax</span>
                <span class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ formatCurrency(bill.tax_total, auth.branchCurrency) }}</span>
              </div>
              <div v-if="bill.discount > 0" class="flex items-center justify-between">
                <span class="text-sm text-gray-500 dark:text-gray-400">Discount</span>
                <span class="text-sm font-medium text-red-600 dark:text-red-400">-{{ formatCurrency(bill.discount, auth.branchCurrency) }}</span>
              </div>
              <div class="border-t border-gray-200 dark:border-gray-700 pt-3">
                <div class="flex items-center justify-between">
                  <span class="text-base font-semibold text-gray-900 dark:text-white">Total</span>
                  <span class="text-lg font-bold text-gray-900 dark:text-white">{{ formatCurrency(bill.total, auth.branchCurrency) }}</span>
                </div>
              </div>
              <div class="border-t border-gray-200 dark:border-gray-700 pt-3">
                <div class="flex items-center justify-between">
                  ...
                  <span class="text-sm font-medium text-emerald-600 dark:text-emerald-400">
                    -{{ formatCurrency((bill.payments || []).reduce((s, p) => s + (Number(p.amount) || 0), 0), auth.branchCurrency) }}
                  </span>
                </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
        ...
        <div v-if="(bill.payments || []).length === 0" class="text-center py-8">
        ...
        <div v-else class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              <tr v-for="(payment, idx) in bill.payments" :key="idx" class="hover:bg-gray-50 dark:hover:bg-gray-700">
