...
// Track selected permissions as a Set of "category:action" strings
...
async function handleSubmit() {
  ...
  try {
    ...
    toastStore.show('Role created successfully')
    router.push({ name: 'SettingsIndex' })
  } catch (error) {
    console.error('Failed to create role:', error)
    const message = error.response?.data?.detail || error.response?.data?.message || 'Failed to create role'
...
<template>
  <div>
    ...
    <div class="max-w-4xl mx-auto space-y-6">
      <!-- Basic Info -->
      <form class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 space-y-6">
