...
async function fetchReport() {
  ...
  try {
    ...
    reportData.value = Array.isArray(data) ? data : data.items || data.invoices || []
    summary.value = data.summary || {
      total_sales: reportData.value.reduce((s, i) => s + (i.total || 0), 0),
      total_tax: reportData.value.reduce((s, i) => s + (i.tax_total || 0), 0),
      net_sales: reportData.value.reduce((s, i) => s + (i.subtotal || 0), 0),
      invoice_count: reportData.value.length,
    }
  } catch (error) {
...
async function exportPdf() {
  ...
  try {
    ...
    const { data } = await getSalesPdf(params)
    downloadBlob(data, 'sales-report.pdf')
  } catch (error) {
...
async function exportExcel() {
  ...
  try {
    const params = { ...filters.value }
    const { data } = await getSalesExcel(params)
    downloadBlob(data, 'sales-report.xlsx')
  } catch (error) {
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="flex items-end">
          <button
            type="button"
            class="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-medium text-white bg-emerald-600 rounded-lg hover:bg-emerald-700 transition-colors"
            @click="applyFilters"
          >
      ...
      <template #cell-status="{ row }">
        <span
          :class="[
            ...
            row.status === 'sent' ? 'bg-blue-100 text-blue-800 dark:bg-blue-900/40 dark:text-blue-400' :
