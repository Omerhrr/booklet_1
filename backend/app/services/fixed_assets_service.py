...
class FixedAssetService:
    ...
    def create(self, asset_data: FixedAssetCreate, business_id: int, branch_id: int = None, user_id: int = None) -> FixedAsset:
        """Create a new fixed asset with journal entry"""
        # Generate asset code if not provided
        asset_code = asset_data.asset_code
        if not asset_code:
            asset_code = self.get_next_asset_code(business_id)
        
        # Handle existing asset with accumulated depreciation
        accumulated_depreciation = Decimal("0")
        if asset_data.is_existing_asset and asset_data.existing_accumulated_depreciation:
            accumulated_depreciation = Decimal(str(asset_data.existing_accumulated_depreciation))
        
        # Calculate book value
        book_value = Decimal(str(asset_data.purchase_cost)) - accumulated_depreciation
        ...
        asset = FixedAsset(
            ...
            salvage_value=Decimal(str(asset_data.salvage_value)) if asset_data.salvage_value else Decimal("0"),
            useful_life_years=asset_data.useful_life_years,
            depreciation_method=asset_data.depreciation_method,
            depreciation_rate=Decimal(str(asset_data.depreciation_rate)) if asset_data.depreciation_rate else None,
        ...
        # Create journal entry to record the asset in the books
        if asset_data.asset_account_id:
            self._create_asset_acquisition_journal(
                asset=asset,
                fund_source_account_id=asset_data.fund_source_account_id,
                is_existing_asset=asset_data.is_existing_asset,
                existing_accumulated_depreciation=accumulated_depreciation,
                branch_id=branch_id,
                user_id=user_id
            )
    ...
    def _create_asset_acquisition_journal(self, asset: FixedAsset, fund_source_account_id: int = None,
                                          is_existing_asset: bool = False, 
                                          existing_accumulated_depreciation: Decimal = Decimal("0"),
                                          branch_id: int = None, user_id: int = None):
        """Create journal entry to record asset acquisition"""
        ...
        if is_existing_asset:
            # For existing assets: 
            # Debit Asset Account (at cost)
            # Credit Opening Balance Equity or Owner's Equity
            description = f"Registration of existing asset - {asset.name}"
            
            jv = JournalVoucher(
                voucher_number=jv_number,
                transaction_date=asset.purchase_date,
                description=description,
                reference=f"ASSET-REG-{asset.asset_code}",
                is_posted=True,
            ...
            # Debit Asset Account (full cost)
            asset_entry = LedgerEntry(
                transaction_date=asset.purchase_date,
            ...
            # If existing accumulated depreciation, record it
            if existing_accumulated_depreciation > 0 and asset.depreciation_account_id:
                # Debit Accumulated Depreciation (to set opening balance)
                acc_dep_entry = LedgerEntry(
                    transaction_date=asset.purchase_date,
                    description=f"Opening accumulated depreciation - {asset.name}",
                    debit=Decimal("0"),
                    credit=existing_accumulated_depreciation,
            ...
            # Credit Opening Balance Equity (for net book value) or Equity account
            # Find or create Opening Balance Equity account
            equity_account = self.db.query(Account).filter(
                Account.business_id == asset.business_id,
                Account.name.ilike('%opening balance equity%')
            ).first()
            
            if not equity_account:
                # Find Owner's Equity as fallback
                equity_account = self.db.query(Account).filter(
                    Account.business_id == asset.business_id,
                    Account.type.ilike('equity')
                ).first()
            
            if equity_account:
                equity_entry = LedgerEntry(
                    transaction_date=asset.purchase_date,
                    description=f"Equity contribution - existing asset {asset.name}",
                    debit=Decimal("0"),
                    credit=asset.purchase_cost - existing_accumulated_depreciation,
            ...
            # Debit Asset Account
            # Credit Fund Source (Cash/Bank/Accounts Payable)
            
            if not fund_source_account_id:
                # Cannot create journal entry without fund source
                return
            ...
            jv = JournalVoucher(
                ...
                transaction_date=asset.purchase_date,
                description=description,
                reference=f"ASSET-{asset.asset_code}",
                is_posted=True,
                branch_id=branch_id or asset.branch_id,
                business_id=asset.business_id,
                created_by=user_id
            ...
            # Credit Fund Source Account (Cash/Bank/Payable)
            fund_entry = LedgerEntry(
                transaction_date=asset.purchase_date,
                description=f"Payment for asset - {asset.name}",
                debit=Decimal("0"),
                credit=asset.purchase_cost,
                account_id=fund_source_account_id,
                journal_voucher_id=jv.id,
                branch_id=branch_id or asset.branch_id
            )
            self.db.add(fund_entry)
