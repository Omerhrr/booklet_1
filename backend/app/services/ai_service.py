...
class AIService:
    ...
    def get_settings(self, business_id: int) -> Optional[AISetting]:
        ...
        return (
            self.db.query(AISetting)
            .filter(AISetting.business_id == business_id)
            .first()
        )
    ...
    def get_user_conversations(
        self, user_id: int, business_id: int, include_archived: bool = False
    ) -> List[AIConversation]:
        """Get all conversations for a user"""
        query = self.db.query(AIConversation).filter(
            AIConversation.user_id == user_id, AIConversation.business_id == business_id
        )
    ...
    async def chat(
        self, user: User, message: str, conversation_id: Optional[int] = None
        ...
        if settings.allow_data_access:
            context_data = self._get_permission_scoped_data(
                user, user_permissions, message
            )
        ...
        try:
            ...
            assistant_msg = AIMessage(
                ...
                data_sources_accessed=json.dumps(list(context_data.keys()))
                if context_data
                else None,
            )
            self.db.add(assistant_msg)
...
Always be helpful, accurate, and security-conscious."""
        ...
        for perm in user_permissions:
            if perm in PERMISSION_DATA_MAPPING:
                allowed_access.append(
                    f"- {PERMISSION_DATA_MAPPING[perm]['description']} (permission: {perm})"
                )
    ...
    def _get_permission_scoped_data(
        self, user: User, user_permissions: set, message: str
    ) -> Dict[str, Any]:
        ...
        # Sales data
        if "invoices:view" in user_permissions and any(
            kw in message_lower for kw in ["sale", "invoice", "revenue", "customer"]
        ):
            data["sales"] = self._get_sales_summary(user.business_id, branch_id)

        # Purchase data
        if "bills:view" in user_permissions and any(
            kw in message_lower for kw in ["purchase", "bill", "vendor", "supplier"]
        ):
            data["purchases"] = self._get_purchases_summary(user.business_id, branch_id)

        # Customer data
        if "customers:view" in user_permissions and any(
            kw in message_lower for kw in ["customer", "client"]
        ):
            data["customers"] = self._get_customers_summary(user.business_id, branch_id)

        # Vendor data
        if "vendors:view" in user_permissions and any(
            kw in message_lower for kw in ["vendor", "supplier"]
        ):
            data["vendors"] = self._get_vendors_summary(user.business_id, branch_id)

        # Inventory data
        if "products:view" in user_permissions and any(
            kw in message_lower for kw in ["product", "inventory", "stock", "item"]
        ):
            data["inventory"] = self._get_inventory_summary(user.business_id, branch_id)

        # Expense data
        if "expenses:view" in user_permissions and any(
            kw in message_lower for kw in ["expense", "cost", "spending"]
        ):
        ...
        if "accounts:view" in user_permissions or "journal:view" in user_permissions:
            if any(
                kw in message_lower
                for kw in ["account", "balance", "ledger", "journal"]
            ):
                data["accounts"] = self._get_accounts_summary(user.business_id)

        # Banking data
        if "banking:view" in user_permissions and any(
            kw in message_lower for kw in ["bank", "account balance", "transaction"]
        ):
            data["banking"] = self._get_banking_summary(user.business_id, branch_id)

        # HR data
        if "employees:view" in user_permissions and any(
            kw in message_lower for kw in ["employee", "staff", "worker"]
        ):
    ...
    async def _call_ai_provider(
        self, settings: AISetting, messages: List[Dict]
    ) -> Dict:
    ...
    async def _call_zai(self, settings: AISetting, messages: List[Dict]) -> Dict:
        ...
        api_key = (
            self._decrypt_api_key(settings.api_key_encrypted)
        ...
        try:
            ...
            raise Exception(
                "Z.ai SDK not installed. Please contact your administrator."
            )
        except Exception as e:
            error_str = str(e)
    ...
    async def _call_gemini(self, settings: AISetting, messages: List[Dict]) -> Dict:
        ...
        try:
            ...
            raise Exception(
                "Google Gemini SDK not installed. Please contact your administrator."
            )
        except Exception as e:
            error_str = str(e)
    ...
    def _get_sales_summary(self, business_id: int, branch_id: Optional[int]) -> Dict:
        ...
        query = self.db.query(
            ...
            func.sum(SalesInvoice.total_amount - SalesInvoice.paid_amount).label(
                "total_outstanding"
            ),
    ...
    def _get_purchases_summary(
        self, business_id: int, branch_id: Optional[int]
    ) -> Dict:
        ...
        query = self.db.query(
            ...
            func.sum(PurchaseBill.total_amount - PurchaseBill.paid_amount).label(
                "total_outstanding"
            ),
    ...
    def _get_inventory_summary(
        ...
        query = self.db.query(
            ...
            func.sum(Product.stock_quantity * Product.purchase_price).label(
                "stock_value"
            ),
    ...
    def _get_accounts_summary(self, business_id: int) -> Dict:
        ...
        accounts = (
            self.db.query(Account).filter(Account.business_id == business_id).all()
        )
    ...
    def _get_employees_summary(
        self, business_id: int, branch_id: Optional[int]
    ) -> Dict:
        """Get employees summary data"""
        query = self.db.query(func.count(Employee.id).label("total_employees")).filter(
            Employee.business_id == business_id, Employee.is_active == True
        )
        ...
        return {"total_active_employees": result.total_employees or 0}
