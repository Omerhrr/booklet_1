...
class BudgetService:
    ...
    def get_budget_vs_actual(self, budget_id: int, business_id: int) -> Dict:
        ...
        For Revenue accounts (credit nature): Actual = Credit - Debit
        For Expense accounts (debit nature): Actual = Debit - Credit
        
        Variance interpretation:
        - Revenue: Positive variance = earned MORE than budgeted (favorable)
        - Expense: Positive variance = spent LESS than budgeted (favorable)
        """
        ...
        for item in budget.items:
            account_type_lower = item.account.type.lower() if item.account and item.account.type else ''
            
            # Calculate actual based on account type
            if account_type_lower == 'revenue':
                # Revenue has credit nature: actual = credit - debit
                ...
                # Expense has debit nature: actual = debit - credit
                actual = self.db.query(
                    func.sum(LedgerEntry.debit - LedgerEntry.credit)
                ).filter(
                    LedgerEntry.account_id == item.account_id,
            ...
            # Calculate variance
            # For revenue: variance = actual - budgeted (positive = favorable, earned more)
            # For expense: variance = budgeted - actual (positive = favorable, spent less)
            if account_type_lower == 'revenue':
                variance = actual - item.amount  # Positive = earned more than budget (favorable)
            else:
                variance = item.amount - actual  # Positive = spent less than budget (favorable)
