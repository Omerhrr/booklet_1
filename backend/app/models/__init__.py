...
class BankStatementLine(Base):
    ...
    transaction_type = Column(String(50), nullable=True)  # debit, credit, transfer, etc.
    description = Column(Text, nullable=True)
    reference = Column(String(100), nullable=True)
    amount = Column(Numeric(15, 2), nullable=False)
    ...
    matched_cashbook_entry_id = Column(Integer, ForeignKey('cash_book_entries.id', ondelete='SET NULL'), nullable=True)
    ...
    # Relationships
    bank_account = relationship("BankAccount")
    matched_cashbook_entry = relationship("CashBookEntry", foreign_keys=[matched_cashbook_entry_id])
    branch = relationship("Branch")
    business = relationship("Business")
...
class BankReconciliationRecord(Base):
    ...
    book_balance = Column(Numeric(15, 2), nullable=False)  # Balance per books at statement date
    
    # Reconciliation items
    deposits_in_transit = Column(Numeric(15, 2), default=Decimal("0.00"))  # Uncleared deposits
    outstanding_checks = Column(Numeric(15, 2), default=Decimal("0.00"))  # Uncleared payments
    other_adjustments = Column(Numeric(15, 2), default=Decimal("0.00"))  # Bank charges, interest, etc.
    adjusted_balance = Column(Numeric(15, 2), nullable=False)  # Calculated adjusted balance
    
    # Status
    status = Column(String(20), default='completed')  # draft, completed, reversed
    difference = Column(Numeric(15, 2), default=Decimal("0.00"))  # Should be 0 if reconciled
    notes = Column(Text, nullable=True)
    ...
    __table_args__ = (
        UniqueConstraint('reconciliation_number', 'business_id', name='uq_reconciliation_number'),
...
class CashBookEntry(Base):
    ...
    matched_statement_line_id = Column(Integer, ForeignKey('bank_statement_lines.id', ondelete='SET NULL'), nullable=True)  # Matched to bank statement line
    ...
    matched_statement_line = relationship("BankStatementLine", foreign_keys=[matched_statement_line_id])
    branch = relationship("Branch")
    business = relationship("Business")
...
class Analysis(Base):
    ...
    data_source = Column(String(50), nullable=False)  # sales, purchases, inventory, etc.
...
class Dashboard(Base):
    ...
    widgets = Column(Text, nullable=True)  # JSON array of widget configs (analysis_id, position, size)
...
class AISetting(Base):
    ...
    # Model configuration
    model_name = Column(String(100), nullable=True)  # e.g., gpt-4, gemini-pro, claude-3-sonnet
    ...
    configured_by = Column(Integer, ForeignKey('users.id', ondelete='SET NULL'), nullable=True)
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    ...
    __table_args__ = (
        Index('ix_ai_settings_business_id', 'business_id'),
    )
...
class AIMessage(Base):
    ...
    data_sources_accessed = Column(Text, nullable=True)  # JSON list of data sources queried
...
class AgentConfiguration(Base):
    ...
    __table_args__ = (
        UniqueConstraint('business_id', 'agent_type', name='uq_agent_config_business_type'),
...
class AgentExecution(Base):
    ...
    id = Column(Integer, primary_key=True)
    business_id = Column(Integer, ForeignKey('businesses.id', ondelete='CASCADE'), nullable=False)
    agent_configuration_id = Column(Integer, ForeignKey('agent_configurations.id', ondelete='CASCADE'), nullable=False)
...
class AgentFinding(Base):
    ...
    # Finding details
    finding_type = Column(String(100), nullable=False)  # discrepancy, error, warning, info
    severity = Column(String(50), default='medium')  # low, medium, high, critical
    title = Column(String(255), nullable=False)
    description = Column(Text, nullable=True)
    
    # Related record
    related_model = Column(String(100), nullable=True)  # e.g., SalesInvoice, PurchaseBill
    related_record_id = Column(Integer, nullable=True)
    
    # Resolution
    resolution_status = Column(String(50), default='open')  # open, acknowledged, resolved, dismissed
...
class DocWizardSession(Base):
    ...
    # Session details
    issue_type = Column(String(100), nullable=True)  # sales_vs_purchase, duplicate_entry, wrong_account, etc.
    ...
    business = relationship("Business")
    user = relationship("User")
    messages = relationship("DocWizardMessage", back_populates="session", cascade="all, delete-orphan")
...
class DocWizardMessage(Base):
    ...
    id = Column(Integer, primary_key=True)
    session_id = Column(Integer, ForeignKey('doc_wizard_sessions.id', ondelete='CASCADE'), nullable=False)
    ...
    __table_args__ = (
        Index('ix_doc_wizard_messages_session_id', 'session_id'),
    )
...
__all__ = [
    # Enums
    'AccountType', 'InvoiceStatus', 'PayFrequency', 'PlanType', 'AssetStatus', 'AgentType', 'AgentStatus',
    # Association Tables
    ...
    'AuditLog',
    # Analytics
    'Analysis', 'Dashboard', 'SavedFilter',
    # AI Assistant
    'AIProvider', 'AISetting', 'AIConversation', 'AIMessage', 'AIUsageLog',
    # Agents
