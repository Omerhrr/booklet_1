...
class Business(Base):
    ...
    sales_invoices = relationship(
        "SalesInvoice", back_populates="business", cascade="all, delete-orphan"
    )
    purchase_bills = relationship(
        "PurchaseBill", back_populates="business", cascade="all, delete-orphan"
    )
    ...
    journal_vouchers = relationship(
        "JournalVoucher", back_populates="business", cascade="all, delete-orphan"
    )
    budgets = relationship(
        "Budget", back_populates="business", cascade="all, delete-orphan"
    )
    fixed_assets = relationship(
        "FixedAsset", back_populates="business", cascade="all, delete-orphan"
    )
...
class User(Base):
    ...
    roles = relationship(
        "UserBranchRole", back_populates="user", cascade="all, delete-orphan"
    )
...
class Branch(Base):
    ...
    user_assignments = relationship(
        "UserBranchRole", back_populates="branch", cascade="all, delete-orphan"
    )
    ...
    __table_args__ = (Index("ix_branches_business_id", "business_id"),)
...
class Role(Base):
    ...
    user_assignments = relationship(
        "UserBranchRole", back_populates="role", cascade="all, delete-orphan"
    )
...
class Account(Base):
    ...
    )  # Store as string to avoid enum mapping issues
    description = Column(Text, nullable=True)
...
class JournalVoucher(Base):
    ...
    ledger_entries = relationship(
        "LedgerEntry", back_populates="journal_voucher", cascade="all, delete-orphan"
    )

    __table_args__ = (
        UniqueConstraint(
            "voucher_number", "business_id", name="uq_journal_voucher_number"
        ),
    )
...
class LedgerEntry(Base):
    ...
    journal_voucher_id = Column(
        Integer, ForeignKey("journal_vouchers.id", ondelete="CASCADE"), nullable=True
    )
    sales_invoice_id = Column(
        Integer, ForeignKey("sales_invoices.id", ondelete="SET NULL"), nullable=True
    )
    purchase_bill_id = Column(
        Integer, ForeignKey("purchase_bills.id", ondelete="SET NULL"), nullable=True
    )
    credit_note_id = Column(
        Integer, ForeignKey("credit_notes.id", ondelete="SET NULL"), nullable=True
    )
    debit_note_id = Column(
        Integer, ForeignKey("debit_notes.id", ondelete="SET NULL"), nullable=True
    )
    expense_id = Column(
        Integer, ForeignKey("expenses.id", ondelete="SET NULL"), nullable=True
    )
    other_income_id = Column(
        Integer, ForeignKey("other_incomes.id", ondelete="SET NULL"), nullable=True
    )
    closing_entry_id = Column(
        Integer, ForeignKey("closing_entries.id", ondelete="SET NULL"), nullable=True
    )
    bad_debt_id = Column(
        Integer, ForeignKey("bad_debts.id", ondelete="SET NULL"), nullable=True
    )
...
class Vendor(Base):
    ...
    __table_args__ = (Index("ix_vendors_business_id", "business_id"),)
...
class Category(Base):
    ...
    __table_args__ = (Index("ix_categories_business_id", "business_id"),)
...
class SalesInvoice(Base):
    ...
    returned_amount = Column(
        Numeric(15, 2), default=Decimal("0.00")
    )  # Amount returned via credit notes
    ...
    items = relationship(
        "SalesInvoiceItem", back_populates="sales_invoice", cascade="all, delete-orphan"
    )
    payments = relationship(
        "Payment", back_populates="sales_invoice", cascade="all, delete-orphan"
    )
    credit_notes = relationship(
        "CreditNote", back_populates="sales_invoice", cascade="all, delete-orphan"
    )
    ledger_entries = relationship("LedgerEntry", back_populates="sales_invoice")

    __table_args__ = (
        UniqueConstraint(
            "invoice_number", "business_id", name="uq_sales_invoice_number"
        ),
...
class SalesInvoiceItem(Base):
    ...
    sales_invoice_id = Column(
        Integer, ForeignKey("sales_invoices.id", ondelete="CASCADE"), nullable=False
    )
    created_at = Column(DateTime, default=datetime.utcnow)
...
class CreditNote(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "credit_note_number", "business_id", name="uq_credit_note_number"
        ),
    )
...
class CreditNoteItem(Base):
    ...
    credit_note_id = Column(
        Integer, ForeignKey("credit_notes.id", ondelete="CASCADE"), nullable=False
    )
    original_item_id = Column(
        Integer,
        ForeignKey("sales_invoice_items.id", ondelete="SET NULL"),
        nullable=True,
    )
    created_at = Column(DateTime, default=datetime.utcnow)
...
class PurchaseBill(Base):
    ...
    returned_amount = Column(
        Numeric(15, 2), default=Decimal("0.00")
    )  # Amount returned via debit notes
    status = Column(String(20), default=InvoiceStatus.PENDING.value)
    notes = Column(Text, nullable=True)
    vendor_id = Column(
    ...
    items = relationship(
        "PurchaseBillItem", back_populates="purchase_bill", cascade="all, delete-orphan"
    )
    debit_notes = relationship(
        "DebitNote", back_populates="purchase_bill", cascade="all, delete-orphan"
    )
...
class PurchaseBillItem(Base):
    ...
    purchase_bill_id = Column(
        Integer, ForeignKey("purchase_bills.id", ondelete="CASCADE"), nullable=False
    )
    created_at = Column(DateTime, default=datetime.utcnow)
...
class DebitNote(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "debit_note_number", "business_id", name="uq_debit_note_number"
        ),
    )
...
class DebitNoteItem(Base):
    ...
    debit_note_id = Column(
        Integer, ForeignKey("debit_notes.id", ondelete="CASCADE"), nullable=False
    )
    original_item_id = Column(
        Integer,
        ForeignKey("purchase_bill_items.id", ondelete="SET NULL"),
        nullable=True,
    )
    created_at = Column(DateTime, default=datetime.utcnow)
...
class OtherIncome(Base):
    ...
    received_in_account_id = Column(
        Integer, ForeignKey("accounts.id", ondelete="SET NULL"), nullable=True
    )
    income_account_id = Column(
        Integer, ForeignKey("accounts.id", ondelete="SET NULL"), nullable=True
    )
    branch_id = Column(
        Integer, ForeignKey("branches.id", ondelete="CASCADE"), nullable=False
...
class Employee(Base):
    ...
    __table_args__ = (Index("ix_employees_business_id", "business_id"),)
...
class BankAccount(Base):
    ...
    __table_args__ = (Index("ix_bank_accounts_business_id", "business_id"),)
...
class FundTransfer(Base):
    ...
    )  # Bank account ID or COA account ID
    ...
    __table_args__ = (
        UniqueConstraint(
            "transfer_number", "business_id", name="uq_fund_transfer_number"
        ),
    )


class BankStatementLine(Base):
    """Bank statement line for reconciliation - imported from bank statement"""
    ...
    )  # debit, credit, transfer, etc.
    description = Column(Text, nullable=True)
    reference = Column(String(100), nullable=True)
    amount = Column(Numeric(15, 2), nullable=False)
    balance_after = Column(Numeric(15, 2), nullable=True)  # Balance shown on statement

    # Reconciliation status
    is_matched = Column(Boolean, default=False)
    is_cleared = Column(Boolean, default=False)
    matched_cashbook_entry_id = Column(
        Integer, ForeignKey("cash_book_entries.id", ondelete="SET NULL"), nullable=True
    )
    matched_date = Column(Date, nullable=True)

    # Import tracking
    statement_id = Column(String(100), nullable=True)  # Bank statement reference
    import_batch_id = Column(String(50), nullable=True)  # Track which import batch
    ...
    # Relationships
    bank_account = relationship("BankAccount")
    matched_cashbook_entry = relationship(
        "CashBookEntry", foreign_keys=[matched_cashbook_entry_id]
    )
    branch = relationship("Branch")
    business = relationship("Business")

    __table_args__ = (
        Index("ix_bank_statement_lines_bank_account_id", "bank_account_id"),
        Index("ix_bank_statement_lines_statement_date", "statement_date"),
        Index("ix_bank_statement_lines_is_cleared", "is_cleared"),
    )


class BankReconciliationRecord(Base):
    """Bank reconciliation record - stores completed reconciliations"""

    __tablename__ = "bank_reconciliation_records"

    id = Column(Integer, primary_key=True)
    reconciliation_number = Column(String(50), nullable=False)
    bank_account_id = Column(
        Integer, ForeignKey("bank_accounts.id", ondelete="CASCADE"), nullable=False
    )
    statement_date = Column(Date, nullable=False)
    ...
    )  # Balance per books at statement date

    # Reconciliation items
    deposits_in_transit = Column(
        Numeric(15, 2), default=Decimal("0.00")
    )  # Uncleared deposits
    ...
    adjusted_balance = Column(
        Numeric(15, 2), nullable=False
    )  # Calculated adjusted balance

    # Status
    status = Column(String(20), default="completed")  # draft, completed, reversed
    difference = Column(
        Numeric(15, 2), default=Decimal("0.00")
    ...
    __table_args__ = (
        UniqueConstraint(
            "reconciliation_number", "business_id", name="uq_reconciliation_number"
        ),
        Index("ix_bank_reconciliation_records_bank_account_id", "bank_account_id"),
    )
...
class Budget(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "name", "fiscal_year", "business_id", name="uq_budget_name_year"
        ),
    )
...
class FixedAsset(Base):
    ...
    # Status & Disposal
    status = Column(
        String(20), default=AssetStatus.ACTIVE.value
    )  # active, disposed, written_off, fully_depreciated
    ...
    depreciation_account_id = Column(
        Integer, ForeignKey("accounts.id", ondelete="SET NULL"), nullable=True
    ...
    depreciation_account = relationship(
        "Account", foreign_keys=[depreciation_account_id]
    )
    expense_account = relationship("Account", foreign_keys=[expense_account_id])
    vendor = relationship("Vendor")
    branch = relationship("Branch")
    ...
    def annual_depreciation(self):
        ...
        if self.depreciation_method == "straight_line":
            ...
            return (
                depreciable_amount / self.useful_life_years
    ...
    def remaining_life(self):
        ...
        if annual_dep > 0:
            remaining_value = (
                self.purchase_cost - self.salvage_value - self.accumulated_depreciation
            )
...
class DepreciationRecord(Base):
    ...
    journal_voucher_id = Column(
        Integer, ForeignKey("journal_vouchers.id", ondelete="SET NULL"), nullable=True
    )
...
class CashBookEntry(Base):
    ...
    source_type = Column(
        String(50), nullable=True
    )  # sales_payment, purchase_payment, expense, income, transfer, manual
    source_id = Column(Integer, nullable=True)  # ID of the source document

    # For transfers
    transfer_id = Column(
        Integer, ForeignKey("fund_transfers.id", ondelete="SET NULL"), nullable=True
    )
    is_transfer = Column(Boolean, default=False)
    transfer_direction = Column(String(10), nullable=True)  # in, out

    is_cleared = Column(Boolean, default=False)  # For bank reconciliation

    matched_statement_line_id = Column(
        Integer,
        ForeignKey("bank_statement_lines.id", ondelete="SET NULL"),
        nullable=True,
    ...
    matched_statement_line = relationship(
        "BankStatementLine", foreign_keys=[matched_statement_line_id]
    )
    branch = relationship("Branch")
    business = relationship("Business")
    ...
    __table_args__ = (
        UniqueConstraint(
            "entry_number", "business_id", name="uq_cash_book_entry_number"
        ),
...
class AuditLog(Base):
    ...
    )  # Store username in case user is deleted
    ...
    )  # Model name: Invoice, Payment, User, etc.
...
class FiscalPeriod(Base):
    ...
    fiscal_year_id = Column(
        Integer, ForeignKey("fiscal_years.id", ondelete="CASCADE"), nullable=False
    )
    ...
    __table_args__ = (
        UniqueConstraint(
            "fiscal_year_id", "period_number", name="uq_fiscal_period_number"
        ),
...
class OpeningBalanceEntry(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "entry_number", "business_id", name="uq_opening_balance_number"
        ),
...
class ClosingEntry(Base):
    ...
    entry_type = Column(
        String(50), nullable=False
    )  # revenue_close, expense_close, income_summary, retained_earnings
    description = Column(Text, nullable=True)
    is_posted = Column(Boolean, default=False)
    posted_at = Column(DateTime, nullable=True)
    posted_by = Column(
...
class BankReconciliationAdjustment(Base):
    ...
    adjustment_type = Column(
        String(50), nullable=False
    )  # bank_charge, interest, error_correction, other
    amount = Column(Numeric(15, 2), nullable=False)
    direction = Column(
    ...
    )  # Link to reconciliation session if applicable
    journal_voucher_id = Column(
        Integer, ForeignKey("journal_vouchers.id", ondelete="SET NULL"), nullable=True
    )
    branch_id = Column(
    ...
    __table_args__ = (
        UniqueConstraint(
            "adjustment_number", "business_id", name="uq_bank_adjustment_number"
        ),
...
class BadDebt(Base):
    ...
    reason = Column(Text, nullable=True)
    status = Column(
        String(20), default="written_off"
    )  # written_off, recovered, partial_recovery
...
class Analysis(Base):
    ...
    columns = Column(Text, nullable=False)  # JSON array of selected columns
    filters = Column(Text, nullable=True)  # JSON array of filter conditions
    group_by = Column(Text, nullable=True)  # JSON array of group by fields
    aggregations = Column(Text, nullable=True)  # JSON object of aggregations
    order_by = Column(Text, nullable=True)  # JSON array of order by clauses

    # Visualization configuration
    chart_type = Column(String(50), default="table")  # table, bar, line, pie, area, kpi
    chart_config = Column(Text, nullable=True)  # JSON object for chart options
    ...
    __table_args__ = (
        Index("ix_analyses_business_id", "business_id"),
        Index("ix_analyses_data_source", "data_source"),
        Index("ix_analyses_created_by", "created_by"),
    )


class Dashboard(Base):
    """Dashboard - collection of analysis widgets"""
    ...
    layout = Column(Text, nullable=True)  # JSON array of widget positions
    widgets = Column(
        Text, nullable=True
    )  # JSON array of widget configs (analysis_id, position, size)
    ...
    __table_args__ = (
        Index("ix_dashboards_business_id", "business_id"),
...
class SavedFilter(Base):
    ...
    filter_config = Column(Text, nullable=False)  # JSON array of filter conditions
    ...
    __table_args__ = (
        Index("ix_saved_filters_business_id", "business_id"),
        Index("ix_saved_filters_data_source", "data_source"),
    )
...
class AISetting(Base):
    """AI configuration settings per business"""
    ...
    provider = Column(String(50), default="zai")  # zai, openai, gemini, claude
    api_key_encrypted = Column(Text, nullable=True)  # Encrypted API key
    api_endpoint = Column(String(255), nullable=True)  # Custom endpoint if needed

    # Model configuration
    model_name = Column(
        String(100), nullable=True
    )  # e.g., gpt-4, gemini-pro, claude-3-sonnet
    max_tokens = Column(Integer, default=4096)
    temperature = Column(Numeric(3, 2), default=Decimal("0.7"))

    # Feature toggles
    is_enabled = Column(Boolean, default=False)
    allow_data_access = Column(Boolean, default=True)  # Allow AI to query business data

    # Rate limiting
    daily_request_limit = Column(Integer, default=1000)
    monthly_request_limit = Column(Integer, default=30000)

    # Ownership - one setting per business
    business_id = Column(
        Integer,
    ...
    __table_args__ = (Index("ix_ai_settings_business_id", "business_id"),)


class AIConversation(Base):
    """AI conversation session - one per user, private"""
    ...
    # Context tracking
    context_summary = Column(Text, nullable=True)  # Summary of conversation for context

    # Ownership - private to user
    user_id = Column(
        Integer, ForeignKey("users.id", ondelete="CASCADE"), nullable=False
    )
    business_id = Column(
    ...
    __table_args__ = (
        ...
        Index("ix_ai_conversations_business_id", "business_id"),
    )
...
class AIMessage(Base):
    ...
    # Data access tracking - what permissions were used for this response
    permissions_used = Column(Text, nullable=True)  # JSON list of permissions used
    data_sources_accessed = Column(
        Text, nullable=True
    )  # JSON list of data sources queried
...
class AIUsageLog(Base):
    ...
    # Request type
    request_type = Column(String(50), default="chat")  # chat, analysis, summary
    ...
    __table_args__ = (
        ...
        Index("ix_ai_usage_logs_business_id", "business_id"),
...
class AgentType(enum.Enum):
    ...
    AUTOMATION = "automation"  # Runs automated tasks (bad debt, depreciation)
    AUDIT = "audit"  # Audits records, generates reports, sends emails
    DOC_WIZARD = "doc_wizard"  # Guides users on fixing issues
...
class AgentConfiguration(Base):
    ...
    # Agent type
    agent_type = Column(String(50), nullable=False)  # automation, audit, doc_wizard

    # Configuration (JSON)
    config = Column(Text, nullable=True)  # JSON config specific to agent type

    # Schedule (for automation and audit agents)
    schedule_enabled = Column(Boolean, default=False)
    schedule_cron = Column(String(100), nullable=True)  # Cron expression for scheduling

    # Email configuration (for audit agent)
    ...
    __table_args__ = (
        UniqueConstraint(
            "business_id", "agent_type", name="uq_agent_config_business_type"
        ),
        Index("ix_agent_configurations_business_id", "business_id"),
        Index("ix_agent_configurations_next_run_at", "next_run_at"),
    )
...
class AgentExecution(Base):
    ...
    # Results
    result_summary = Column(Text, nullable=True)  # Summary of what was done
    result_details = Column(Text, nullable=True)  # JSON with detailed results
    report_path = Column(String(500), nullable=True)  # Path to generated PDF report
    ...
    records_flagged = Column(Integer, default=0)  # Issues found

    # Branch-specific (for audit agent)
    branch_id = Column(
        Integer, ForeignKey("branches.id", ondelete="SET NULL"), nullable=True
    )
    ...
    # Relationships
    business = relationship("Business")
    agent_configuration = relationship("AgentConfiguration", backref="executions")
    branch = relationship("Branch")

    __table_args__ = (
        Index("ix_agent_executions_business_id", "business_id"),
...
class AgentFinding(Base):
    """Findings/issues discovered by agents"""
    ...
    execution_id = Column(
        Integer, ForeignKey("agent_executions.id", ondelete="CASCADE"), nullable=False
    )

    # Finding details
    finding_type = Column(
        String(100), nullable=False
    )  # discrepancy, error, warning, info
    ...
    # Relationships
    business = relationship("Business")
    execution = relationship("AgentExecution", backref="findings")
    branch = relationship("Branch")
    resolver = relationship("User")

    __table_args__ = (
        Index("ix_agent_findings_business_id", "business_id"),
        Index("ix_agent_findings_execution_id", "execution_id"),
        Index("ix_agent_findings_severity", "severity"),
        Index("ix_agent_findings_resolution_status", "resolution_status"),
    )


class DocWizardSession(Base):
    """Session for Doc Wizard interactions"""

    __tablename__ = "doc_wizard_sessions"
    ...
    issue_type = Column(
        String(100), nullable=True
    )  # sales_vs_purchase, duplicate_entry, wrong_account, etc.
    description = Column(Text, nullable=True)  # User's description of the issue
    ...
    user = relationship("User")
    messages = relationship(
        "DocWizardMessage", back_populates="session", cascade="all, delete-orphan"
    )

    __table_args__ = (
        Index("ix_doc_wizard_sessions_business_id", "business_id"),
        Index("ix_doc_wizard_sessions_user_id", "user_id"),
    )


class DocWizardMessage(Base):
    """Messages in Doc Wizard sessions"""

    __tablename__ = "doc_wizard_messages"

    id = Column(Integer, primary_key=True)
    session_id = Column(
        Integer,
        ForeignKey("doc_wizard_sessions.id", ondelete="CASCADE"),
        nullable=False,
    )
    ...
    # Relationships
    session = relationship("DocWizardSession", back_populates="messages")

    __table_args__ = (Index("ix_doc_wizard_messages_session_id", "session_id"),)
