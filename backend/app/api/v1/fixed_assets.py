...
router = APIRouter(prefix="/fixed-assets", tags=["Fixed Assets"], dependencies=[Depends(PlanFeatureChecker("fixed_assets"))])
...
async def create_fixed_asset(
    ...
    asset_service = FixedAssetService(db)
    branch_id = current_user.selected_branch.id if current_user.selected_branch else None
    asset = asset_service.create(asset_data, current_user.business_id, branch_id, user_id=current_user.id)
    db.commit()
    return asset
