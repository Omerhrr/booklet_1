...
async def verify_service_api_key(x_api_key: str = Header(None)):
    """Verify internal service API key for inter-service calls"""
    valid_key = os.environ.get("SERVICE_API_KEY", "booklet-internal-service-key")
...
async def require_admin(
    ...
    website_user = (
        db.query(WebsiteUser).filter(WebsiteUser.user_id == current_user.id).first()
    )
    if not (
        current_user.is_superuser or (website_user and website_user.is_website_admin)
    ):
...
class RegisterRequest(BaseModel):
    ...
    plan_slug: str = Field(
        default="basic", pattern="^(basic|premium|advanced|enterprise)$"
    )
...
def get_subscription(
    ...
    if not business_id or business_id != current_user.business_id:
...
def register(data: RegisterRequest, db: Session = Depends(get_db)):
    ...
    plan = (
        db.query(SubscriptionPlan)
        .filter(SubscriptionPlan.slug == data.plan_slug)
        .first()
    )
    if not plan:
    ...
    for perm in permissions:
        role_perm = RolePermission(role_id=owner_role.id, permission_id=perm.id)
        db.add(role_perm)

    # Assign role to user
    user_branch_role = UserBranchRole(
        user_id=user.id, branch_id=branch.id, role_id=owner_role.id
    )
    ...
    if not plan:
        raise HTTPException(
            status_code=400, detail="Could not create subscription plan"
        )
    subscription = Subscription(
        ...
        + timedelta(days=30 if data.billing_cycle == "monthly" else 365),
    )
    db.add(subscription)
    ...
    audit_service.log(
        ...
        user_id=user.id,
        business_id=business.id,
        description=f"SaaS registration: user={user.username}, business={business.name}",
    )
...
def login(data: dict, db: Session = Depends(get_db)):
    ...
    # Check subscription and get plan limits
    ...
    subscription = (
        ...
        .filter(Subscription.business_id == user.business_id)
        .first()
    )
    ...
    # Create access token with same structure as ERP login
    # Use username (not email) as sub, and include business_id
    ...
    return {
        "user": {
            ...
            "is_website_admin": website_user.is_website_admin
            if website_user
            else False,
...
def verify_token(data: dict, db: Session = Depends(get_db)):
    """Verify JWT token and return user info - used for auto-login"""
    ...
    # Get user info from payload - handle both old and new token formats
    # Old format: sub=email, user_id=id
    # New format: sub=username, business_id=business_id
    user_id = payload.get("user_id")
    sub = payload.get("sub")  # Could be email (old) or username (new)

    if not sub:
        raise HTTPException(status_code=401, detail="Invalid token payload")

    # Look up user by ID (old format) or by username/email (new format)
    user = None
    if user_id:
        # Old format - look up by user_id
        user = db.query(User).filter(User.id == user_id).first()
    else:
        # New format - sub is username, try that first, then email
        user = db.query(User).filter(User.username == sub).first()
        if not user:
            # Fallback: try looking up by email (for backwards compatibility)
    ...
    # Generate new access token with same structure as regular login
    # Use username (not email) as sub, and include business_id
    access_token = create_access_token(
        data={"sub": user.username, "business_id": user.business_id}
    )
...
def update_blog_post(
    ...
    if data.slug:
        ...
        existing = (
            db.query(BlogPost)
            .filter(BlogPost.slug == data.slug, BlogPost.id != post_id)
            .first()
        )
        if existing:
            raise HTTPException(status_code=400, detail="Slug already exists")
...
def delete_blog_post(
    post_id: int, admin=Depends(require_admin), db: Session = Depends(get_db)
):
    """Delete a blog post"""
    post = db.query(BlogPost).filter(BlogPost.id == post_id).first()
    if not post:
...
def get_section_content(section: str, db: Session = Depends(get_db)):
    ...
    contents = (
        ...
        .filter(WebsiteContent.section == section, WebsiteContent.is_active == True)
        .all()
    )
...
def update_section_content(
    ...
    for key, value in data.items():
        content = (
            db.query(WebsiteContent)
            .filter(WebsiteContent.section == section, WebsiteContent.key == key)
            .first()
        )
        ...
        if content:
            ...
            content = WebsiteContent(
                section=section, key=key, content=json.dumps(value)
            )
            db.add(content)
...
def get_payments(
    ...
    payments = (
        ...
        .filter(PaymentHistory.subscription_id == subscription.id)
...
def get_usage(
    ...
    return {
        "usage": {
            ...
            "users": {"used": user_count, "limit": plan.max_users if plan else 5},
...
def get_plan_limits(
    business_id: int = None,
    db: Session = Depends(get_db),
    _auth=Depends(verify_service_api_key),
):
    """Get plan limits for a business - used by ERP to enforce restrictions"""
    if not business_id:
        return {
            "plan": {
                "max_branches": 1,
...
@router.get("/plan-limits/{business_id}")
def get_plan_limits_by_id(
    business_id: int,
    current_user=Depends(get_current_user),
    db: Session = Depends(get_db),
):
    """Get plan limits for a business - authenticated endpoint for frontend"""
    if current_user.business_id != business_id:
        raise HTTPException(
            status_code=403, detail="Cannot access other business plan limits"
        )
...
@router.get("/software-logins")
def get_software_logins(
    business_id: int = None,
    db: Session = Depends(get_db),
    current_user=Depends(get_current_user),
):
    """Get software login info for users in a business"""
    if not business_id or business_id != current_user.business_id:
        return {"logins": []}
...
def get_admin_stats(admin=Depends(require_admin), db: Session = Depends(get_db)):
    ...
    new_contacts = (
        db.query(ContactSubmission).filter(ContactSubmission.status == "new").count()
    )
...
def get_recent_registrations(
    limit: int = 10, admin=Depends(require_admin), db: Session = Depends(get_db)
):
    """Get recent registrations"""
    businesses = (
        db.query(Business).order_by(desc(Business.created_at)).limit(limit).all()
    )
