...
def login(data: dict, db: Session = Depends(get_db)):
    ...
    plan_limits = {"max_branches": 1, "max_users": 5, "includes_agents": False, "slug": "basic"}
    ...
    access_token = create_access_token(data={"sub": user.username, "business_id": user.business_id})
...
def get_plan_limits(business_id: int = None, db: Session = Depends(get_db)):
    """Get plan limits for a business - used by ERP to enforce restrictions"""
    if not business_id:
        return {"plan": {"max_branches": 1, "max_users": 5, "includes_agents": False, "slug": "basic"}}
...
def get_software_logins(business_id: int = None, db: Session = Depends(get_db)):
