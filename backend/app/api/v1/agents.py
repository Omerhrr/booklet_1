...
router = APIRouter(
    prefix="/agents",
    tags=["Agents"],
    dependencies=[Depends(PlanFeatureChecker("agents"))],
)
        ...
        "configurations": [
            {
                ...
                "email_recipients": json.loads(c.email_recipients)
                if c.email_recipients
                else [],
...
async def get_configuration(
    ...
    return {
        ...
        "email_recipients": json.loads(config.email_recipients)
...
async def list_executions(
    ...
    perm_service = PermissionService(db)
    if not perm_service.user_has_permission(current_user, "agents:view_findings"):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="You don't have permission to view agent executions",
        )
    ...
    return {
        "executions": [
            {
                "id": e.id,
                "agent_type": e.agent_configuration.agent_type
                if e.agent_configuration
                else None,
                "status": e.status,
...
async def get_execution(
    ...
    return {
        "execution": {
            "id": execution.id,
            "agent_type": execution.agent_configuration.agent_type
            if execution.agent_configuration
            else None,
            "status": execution.status,
            ...
            "result_details": json.loads(execution.result_details)
            if execution.result_details
            else None,
            "records_processed": execution.records_processed,
...
async def list_findings(
    ...
    if not perm_service.user_has_permission(current_user, "agents:view_findings"):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="You don't have permission to view agent findings",
        )
...
async def resolve_finding(
    ...
    if not perm_service.user_has_permission(current_user, "agents:view_findings"):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="You don't have permission to resolve findings",
        )
    ...
    try:
        ...
        return {
            ...
            if not request.dismiss
            else "Finding dismissed",
        }
...
async def create_wizard_session(
    ...
    perm_service = PermissionService(db)
    if not perm_service.user_has_permission(current_user, "doc_wizard:use"):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="You don't have permission to use the Doc Wizard",
        )
...
async def list_wizard_sessions(
    ...
    sessions = service.get_user_sessions(
        user_id=current_user.id, business_id=current_user.business_id, limit=limit
    )
            ...
            "messages": [
                {
                    ...
                    "role": m.role,
                    "content": m.content,
                    "suggested_actions": json.loads(m.suggested_actions)
...
async def add_wizard_message(
    ...
    # Add user message
    service.add_message(session_id=session_id, role="user", content=request.content)
