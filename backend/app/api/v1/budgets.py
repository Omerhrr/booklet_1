...
from app.core.security import get_current_active_user, PermissionChecker, PlanFeatureChecker
...
router = APIRouter(prefix="/budgets", tags=["Budgets"], dependencies=[Depends(PlanFeatureChecker("budgets"))])
...
async def get_budget_vs_actual(
    ...
    Returns variance with proper sign:
    ...
    for item in items:
        ...
        # Calculate variance percentage based on budgeted amount
        variance_percent = float(variance / budgeted * 100) if budgeted != 0 else 0
        
        # Determine if variance is favorable
        # For revenue: positive variance = favorable (earned more)
        # For expense: positive variance = favorable (spent less)
