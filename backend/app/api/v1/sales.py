...
async def list_invoices(
    ...
    invoices = sales_service.get_by_branch(
        current_user.selected_branch.id, current_user.business_id, status
    )
...
@router.post(
    "/invoices",
    response_model=SalesInvoiceWithItems,
    dependencies=[Depends(PermissionChecker(["sales:create"]))],
)
async def create_invoice(
    ...
    sales_service = SalesService(db)
    vat_rate = (
        current_user.business.vat_rate if current_user.business.is_vat_registered else 0
    )
...
async def get_invoice(
    ...
    invoice = sales_service.get_by_id(
        invoice_id, current_user.business_id, current_user.selected_branch.id
    )
    if not invoice:
    ...
    invoice_dict["customer_email"] = (
        invoice.customer.email if invoice.customer else None
    )
    ...
    if (
        "returned_amount" not in invoice_dict
        or invoice_dict.get("returned_amount") is None
    ):
...
@router.post(
    ...
    dependencies=[Depends(PermissionChecker(["sales:edit"]))],
)
async def record_payment(
    ...
    try:
        ...
        return {
            ...
            "invoice": SalesInvoiceResponse.model_validate(invoice),
        }
...
@router.post(
    ...
    dependencies=[Depends(PermissionChecker(["sales:delete"]))],
)
...
async def list_credit_notes(
    ...
    credit_notes = cn_service.get_by_branch(
        current_user.selected_branch.id, current_user.business_id
    )
...
async def get_credit_note(
    ...
    cn = cn_service.get_by_id(
        credit_note_id, current_user.business_id, current_user.selected_branch.id
    )
    if not cn:
...
@router.post(
    "/credit-notes", dependencies=[Depends(PermissionChecker(["credit_notes:create"]))]
)
async def create_credit_note(
    ...
    cn = cn_service.create_for_invoice(
        invoice,
        items_to_return,
        credit_note_data.credit_note_date,
...
@router.post(
    ...
    dependencies=[Depends(PermissionChecker(["credit_notes:edit"]))],
)
...
async def list_bad_debts(
    ...
    query = (
        db.query(BadDebt)
        .options(joinedload(BadDebt.customer), joinedload(BadDebt.sales_invoice))
        .filter(
        ...
        result.append(
            {
                ...
                "write_off_date": bd.write_off_date.isoformat()
                if bd.write_off_date
                else None,
                ...
                "invoice_number": bd.sales_invoice.invoice_number
                if bd.sales_invoice
                else None,
...
async def get_bad_debt(
    ...
    bd = (
        ...
        .filter(
            BadDebt.id == bad_debt_id, BadDebt.business_id == current_user.business_id
        )
        .first()
    )
...
async def get_bad_debt_summary(
    ...
    # Total bad debts
    total_bad_debts = (
        db.query(func.sum(BadDebt.amount))
        .filter(
            BadDebt.business_id == current_user.business_id,
            BadDebt.branch_id == current_user.selected_branch.id,
        )
    ...
    # Total recovered
    total_recovered = (
        db.query(func.sum(BadDebt.recovered_amount))
        .filter(
            BadDebt.business_id == current_user.business_id,
            BadDebt.branch_id == current_user.selected_branch.id,
        )
    ...
    # Count by status
    status_counts = (
        db.query(BadDebt.status, func.count(BadDebt.id))
        .filter(
            BadDebt.business_id == current_user.business_id,
            BadDebt.branch_id == current_user.selected_branch.id,
        )
        .group_by(BadDebt.status)
        .all()
    )
