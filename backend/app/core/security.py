...
async def get_current_active_user(
    ...
    if current_user.is_superuser:
        accessible_branches = branch_service.get_branches_by_business(
    ...
    if selected_branch_id:
        try:
            ...
            selected_branch = next(
                (b for b in accessible_branches if b.id == selected_branch_id), None
            )
    ...
    if not selected_branch:
        ...
        selected_branch = next(
            (b for b in accessible_branches if b.is_default), accessible_branches[0]
        )
...
class PermissionChecker:
    ...
    def __call__(
        self, user=Depends(get_current_active_user), db: Session = Depends(get_db)
    ):
...
class PlanLimitChecker:
    ...
    def __call__(
        ...
        if self.resource_type == "branches":
            current_count = (
                db.query(Branch).filter(Branch.business_id == user.business_id).count()
            )
        elif self.resource_type == "users":
            current_count = (
                db.query(User).filter(User.business_id == user.business_id).count()
            )
        else:
            current_count = 0
