...
=========================
This script performs NON-DESTRUCTIVE migrations on the SQLite database.
It will NEVER delete your data. Instead, it:
- Adds missing columns using ALTER TABLE ADD COLUMN
- Creates missing tables using CREATE TABLE IF NOT EXISTS
- Updates data values to fix type issues
- Creates missing indexes

Your data is always preserved. A backup is created automatically before migration.
    ...
    if not table_exists('bank_statement_lines'):
        ...
        cursor.execute("""
            CREATE TABLE bank_statement_lines (
                ...
                matched_cashbook_entry_id INTEGER REFERENCES cash_book_entries(id) ON DELETE SET NULL,
                matched_date DATE,
    ...
    if not table_exists('bank_reconciliation_records'):
        ...
        cursor.execute("""
            CREATE TABLE bank_reconciliation_records (
                ...
                deposits_in_transit NUMERIC(15, 2) DEFAULT 0.00,
                outstanding_checks NUMERIC(15, 2) DEFAULT 0.00,
                other_adjustments NUMERIC(15, 2) DEFAULT 0.00,
    ...
    if not table_exists('bank_reconciliation_records'):
        ...
        cursor.execute("""
            CREATE TABLE bank_reconciliation_records (
                ...
                calculated_book_balance NUMERIC(15, 2),
    ...
    if not table_exists('ai_settings'):
        ...
        cursor.execute("""
            CREATE TABLE ai_settings (
                id INTEGER PRIMARY KEY,
                provider VARCHAR(50) DEFAULT 'zai',
                ...
                allow_data_access BOOLEAN DEFAULT 1,
