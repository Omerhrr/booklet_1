...
def list_customers():
    ...
    customers, status = api_request(
        "GET", f"/crm/customers?include_inactive=true&page={page}&per_page={per_page}"
    )
...
def new_customer():
    ...
    error = (
        response.get("detail", "Failed to create customer")
        if response
...
def edit_customer(customer_id):
    ...
    if request.method == "GET":
        ...
        if status != 200:
            ...
            return redirect(url_for("crm.list_customers"))
        return render_template(
            "crm/customer_form.html", title="Edit Customer", customer=customer
        )
    ...
    error = (
        response.get("detail", "Failed to update customer")
        if response
        else "Failed to update customer"
    )
...
def list_vendors():
    ...
    page = int(request.args.get("page", 1))
    per_page = int(request.args.get("per_page", 20))
    vendors, status = api_request(
        "GET", f"/crm/vendors?include_inactive=true&page={page}&per_page={per_page}"
    )
...
def new_vendor():
    ...
    error = (
        response.get("detail", "Failed to create vendor")
        if response
        else "Failed to create vendor"
    )
...
def edit_vendor(vendor_id):
    ...
    if request.method == "GET":
        ...
        if status != 200:
            ...
            return redirect(url_for("crm.list_vendors"))
        return render_template(
            "crm/vendor_form.html", title="Edit Vendor", vendor=vendor
        )
    ...
    error = (
        response.get("detail", "Failed to update vendor")
        if response
        else "Failed to update vendor"
    )
    flash(error, "error")
...
def toggle_customer_status(customer_id):
    ...
    response, status = api_request(
        "POST", f"/crm/customers/{customer_id}/toggle-status"
    )
