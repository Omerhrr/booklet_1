...
def login():
    ...
    if status == 200 and 'access_token' in response:
        ...
        if user_data:
            ...
            session['business_id'] = user_data.get('business_id')
        
        # Fetch user permissions (already filtered by plan in backend)
        # Also fetches plan_slug and business_id from the API
        perms_data, _ = api_request('GET', '/auth/permissions')
        if perms_data:
            # Store permissions (already filtered by backend)
            session['permissions'] = perms_data.get('permissions', [])
            
            # Use plan_slug from API response
            plan_slug = perms_data.get('plan_slug', 'basic')
            business_id = perms_data.get('business_id')
            
            # Update business_id if provided in API response
            if business_id:
                session['business_id'] = business_id
        
        # Fetch plan limits for UI checks
        business_id = session.get('business_id')
        if business_id:
            plan_data, _ = api_request('GET', f'/saas/plan-limits?business_id={business_id}')
            ...
            # No business_id - set default plan limits
            session['plan_limits'] = {
                'max_branches': 1,
                'max_users': 5,
                'includes_agents': False,
...
def signup():
    ...
    if status == 200 and 'access_token' in response:
        ...
        if perms_data:
            ...
            # Update business_id and plan_slug from API response
            if perms_data.get('business_id'):
                session['business_id'] = perms_data.get('business_id')
            
            plan_slug = perms_data.get('plan_slug', 'basic')
        else:
            plan_slug = 'basic'
        
        # Set plan limits for new accounts (default to basic)
        session['plan_limits'] = {
            'max_branches': 1,
            'max_users': 5,
            'includes_agents': False,
...
def refresh_permissions():
    ...
    if perms_data:
        ...
        # Update plan_slug and business_id from API response
        plan_slug = perms_data.get('plan_slug', 'basic')
        business_id = perms_data.get('business_id')
        ...
        # Update plan_limits in session if we got plan_slug
        current_plan_limits = session.get('plan_limits', {})
        current_plan_limits['slug'] = plan_slug
        session['plan_limits'] = current_plan_limits
        
        session.modified = True
        
        print(f"[DEBUG] User: {session.get('username')}")
        print(f"[DEBUG] Business ID: {session.get('business_id')}")
        print(f"[DEBUG] Plan: {plan_slug}")
        print(f"[DEBUG] Permissions count: {len(session.get('permissions', []))}")
        
        # Check for debug info from API
