...
def check_plan_feature():
    """Check if Agents feature is available for the current plan"""
    plan_limits = session.get(
        "plan_limits", {"slug": "basic", "includes_agents": False}
    )
...
bp.before_request(check_plan_feature)
...
def automation():
    ...
    # Get recent executions
    executions, _ = api_request(
        "GET", "/agents/executions?agent_type=automation&limit=20"
    )

    return render_template(
        "agents/automation.html",
...
def audit():
    ...
    findings_summary = {"critical": 0, "high": 0, "medium": 0, "low": 0, "open": 0}
...
def run_audit():
    ...
    if status_code == 200:
        flash(
            f"Audit completed: Processed {result.get('records_processed', 0)} records, found {result.get('records_flagged', 0)} issues",
            "success",
        )
    else:
...
def wizard():
    ...
    return render_template(
        "agents/wizard.html", title="Doc Wizard", sessions=sessions.get("sessions", [])
    )
...
def create_wizard_session():
    ...
    if status_code == 200:
        ...
        return jsonify(
            {
                "success": False,
                "error": result.get("detail", "Failed to create session"),
            }
        ), 400
...
def add_wizard_message(session_id):
    ...
    data = {"content": request.json.get("content")}

    result, status_code = api_request(
        "POST", f"/agents/wizard/sessions/{session_id}/messages", data=data
    )
...
def resolve_wizard_session(session_id):
    ...
    result, status_code = api_request(
        "POST",
        f"/agents/wizard/sessions/{session_id}/resolve?resolution_summary={resolution_summary}",
    )
...
def findings():
    ...
    findings_result, _ = api_request(
        "GET",
        f"/agents/findings?{'&'.join(params)}"
        if params
        else "/agents/findings?limit=100",
    )

    return render_template(
        "agents/findings.html",
...
def resolve_finding(finding_id):
    ...
    result, status_code = api_request(
        "POST", f"/agents/findings/{finding_id}/resolve", data=data
    )
