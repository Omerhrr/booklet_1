...
@bp.before_request
@login_required
def check_hr_feature():
    """Check if HR feature is available for the current plan"""
...
def new_employee():
    ...
    error = (
        response.get("detail", "Failed to create employee") if response else "Failed"
    )
    flash(error, "error")
...
def edit_employee(employee_id):
    ...
    if request.method == "GET":
        ...
        if status != 200:
            ...
            return redirect(url_for("hr.list_employees"))
        return render_template(
            "hr/employee_form.html", title="Edit Employee", employee=employee
        )
    ...
    error = (
        response.get("detail", "Failed to update employee") if response else "Failed"
    )
    flash(error, "error")
...
def edit_payroll_config(employee_id):
    ...
    if request.method == "POST":
        ...
        existing_config, _ = api_request(
            "GET", f"/hr/employees/{employee_id}/payroll-config"
        )

        if existing_config and "id" in existing_config:
            response, status = api_request(
                "PUT", f"/hr/employees/{employee_id}/payroll-config", data=data
            )
        else:
            response, status = api_request(
                "POST", f"/hr/employees/{employee_id}/payroll-config", data=data
            )
        ...
        error = (
            response.get("detail", "Failed to save payroll configuration")
            if response
            else "Failed"
        )
        flash(error, "error")
...
def terminate_employee(employee_id):
    ...
    response, status = api_request(
        "POST",
        f"/hr/employees/{employee_id}/terminate?termination_date={termination_date}",
    )
    ...
    if status == 200:
        ...
        error = (
            response.get("detail", "Failed to terminate employee")
            if response
            else "Failed"
        )
        flash(error, "error")
...
def run_payroll():
    ...
    if request.method == "POST":
        ...
        if pay_period_start and pay_period_end:
            ...
            if status == 200:
                flash(
                    response.get("message", "Payroll completed successfully"), "success"
                )
                return redirect(url_for("hr.payslips"))
            else:
                error = (
                    response.get("detail", "Failed to run payroll")
                    if response
                    else "Failed"
                )
                flash(error, "error")
    ...
    return render_template(
        "hr/run_payroll.html", title="Run Payroll", employees=employees or []
    )
...
def mark_payslip_paid(payslip_id):
    ...
    if status == 200:
        ...
        error = (
            response.get("detail", "Failed to mark payslip as paid")
            if response
            else "Failed"
        )
        flash(error, "error")
...
def payslip_pdf(payslip_id):
    ...
    response.headers["Content-Disposition"] = (
        f"attachment; filename=payslip_{payslip.get('payslip_number', payslip_id)}.pdf"
    )
    return response
...
def payslips_excel():
    ...
    for row, p in enumerate(payslips, 12):
        ...
        ws.cell(
            row=row, column=10, value=p.get("status", "pending")
        ).border = thin_border
    ...
    response.headers["Content-Disposition"] = (
        f"attachment; filename=payslips_{date.today()}.xlsx"
    )
    return response
...
def payroll_summary():
    ...
    summary, status = api_request(
        "GET",
        f"/hr/payroll/summary?pay_period_start={pay_period_start}&pay_period_end={pay_period_end}",
    )
