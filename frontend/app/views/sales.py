...
def list_invoices():
    ...
    params = (
        ...
        if status_filter
        else {"page": page, "per_page": per_page}
    )
    ...
    total = len(invoices) if isinstance(invoices, list) else 0
        ...
        items.append(
            {
                ...
                "quantity": float(
                    request.form.get(f"items[{item_index}][quantity]", 0)
                ),
...
def record_payment(invoice_id):
    ...
    response, status = api_request(
        "POST", f"/sales/invoices/{invoice_id}/payment", data=data
    )
...
def write_off_invoice(invoice_id):
    ...
    response, status = api_request(
        "POST", f"/sales/invoices/{invoice_id}/write-off", data=data
    )
    ...
    error = (
        response.get("detail", "Failed to write off invoice") if response else "Failed"
    )
    flash(error, "error")
    return redirect(url_for("sales.view_invoice", invoice_id=invoice_id))
...
def list_credit_notes():
    ...
    return render_template(
        "sales/credit_notes.html", title="Credit Notes", credit_notes=credit_notes
    )
...
def create_credit_note(invoice_id):
            ...
            items_to_return.append(
                {
                    "original_item_id": int(
                        request.form.get(f"items[{item_index}][item_id]")
                    ),
                    "product_id": int(
                        request.form.get(f"items[{item_index}][product_id]")
                    ),
                    "quantity": quantity,
                    "price": float(request.form.get(f"items[{item_index}][price]", 0)),
                }
    ...
    if status == 200:
        ...
        return redirect(
            url_for("sales.view_credit_note", credit_note_id=response.get("id"))
        )

    error = (
        response.get("detail", "Failed to create credit note") if response else "Failed"
    )
    flash(error, "error")
    return redirect(url_for("sales.create_credit_note", invoice_id=invoice_id))
...
def apply_credit_note(credit_note_id):
    ...
    response, status = api_request(
        "POST", f"/sales/credit-notes/{credit_note_id}/apply", data=data
    )
    ...
    if status == 200:
        ...
        if refund_amount > 0 and refund_method_used:
            if refund_method_used == "customer_balance":
                flash(
                    f"Credit note applied successfully. {refund_amount} added to customer balance.",
                    "success",
                )
            elif refund_method_used == "cash":
                flash(
                    f"Credit note applied successfully. Refund of {refund_amount} processed.",
                    "success",
                )
            else:
            ...
            flash("Credit note applied to invoice successfully", "success")
        return redirect(
            url_for("sales.view_credit_note", credit_note_id=credit_note_id)
        )

    error = (
        response.get("detail", "Failed to apply credit note") if response else "Failed"
    )
    flash(error, "error")
