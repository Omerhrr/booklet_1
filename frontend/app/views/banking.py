...
from app import api_request, login_required, permission_required, CURRENCY_SYMBOLS
...
def list_accounts():
    ...
    return render_template(
        "banking/accounts.html", title="Bank Accounts", accounts=accounts
    )
...
def new_account():
    ...
    error = (
        response.get("detail", "Failed to create bank account")
        if response
        else "Failed"
    )
    flash(error, "error")
...
def view_account(account_id):
    ...
    transactions_data, _ = api_request(
        "GET",
...
def delete_account(account_id):
    ...
    if status == 200:
        ...
        error = (
            response.get("detail", "Failed to delete bank account")
            if response
...
def new_transfer():
    ...
    error = (
        response.get("detail", "Failed to create transfer") if response else "Failed"
    )
    flash(error, "error")
    return redirect(url_for("banking.transfers"))
...
def reconciliation():
    ...
    if account_id:
        ...
        selected_account, acc_status = api_request(
            "GET", f"/banking/accounts/{account_id}"
        )
        if acc_status == 200:
            # Get reconciliation status
            recon_status, _ = api_request(
                "GET", f"/reconciliation/accounts/{account_id}/status"
            )
            # Get statement lines (uncleared)
            statement_lines, _ = api_request(
                "GET",
                f"/reconciliation/accounts/{account_id}/statement-lines",
                params={"is_cleared": "false"},
            )
            # Get uncleared cashbook transactions
            uncleared_transactions, _ = api_request(
                "GET", f"/reconciliation/accounts/{account_id}/uncleared-transactions"
            )
            # Get reconciliation history
            recon_history, _ = api_request(
                "GET",
                f"/reconciliation/history",
...
def process_reconciliation(account_id):
    ...
    if status == 200:
        ...
        error = (
            response.get("detail", "Failed to complete reconciliation")
            if response
            else "Failed"
        )
        flash(error, "error")
...
def import_statement(account_id):
    ...
    backend_url = f"{current_app.config['BACKEND_URL']}/api/v1/reconciliation/accounts/{account_id}/import-statement"
    headers = {"Authorization": f"Bearer {session.get('access_token')}"}

    files = {"file": (file.filename, content, "text/csv")}
    params = {"statement_date": statement_date, "has_header": str(has_header).lower()}

    try:
        response = http_requests.post(
            backend_url, headers=headers, files=files, params=params
        )
...
def match_transactions():
    ...
    if status == 200:
        ...
        error = (
            response.get("detail", "Failed to match transactions")
            if response
            else "Failed"
        )
        flash(error, "error")
...
def auto_match_transactions(account_id):
    ...
    if status == 200:
        result = response
        flash(
            f"Auto-matched {len(result.get('matched_pairs', []))} transaction pairs",
            "success",
        )
    else:
...
def clear_statement_line(line_id):
    ...
    response, status = api_request(
        "POST", f"/reconciliation/clear-statement-line/{line_id}"
    )
...
def clear_cashbook_entry(entry_id):
    ...
    response, status = api_request(
        "POST", f"/reconciliation/clear-cashbook-entry/{entry_id}"
    )
...
def download_statement_pdf(account_id):
    ...
    url = f"{current_app.config['BACKEND_URL']}/api/v1/banking/accounts/{account_id}/statement/pdf"
    ...
    try:
        response = http_requests.get(url, headers=headers, stream=True)
        return (
            response.content,
            200,
            {
...
def download_statement_excel(account_id):
    ...
    url = f"{current_app.config['BACKEND_URL']}/api/v1/banking/accounts/{account_id}/statement/excel"
