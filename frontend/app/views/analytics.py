...
def get_source_fields(source_id):
    ...
    if status_code == 200:
        ...
        return jsonify(
            {"error": fields.get("detail", "Failed to load fields")}
        ), status_code
...
def saved_analyses():
    ...
    analyses, _ = api_request(
        "GET", "/analytics/analyses", params=params if params else None
    )

    return render_template(
        "analytics/saved.html",
...
def new_analysis():
    ...
    if request.method == "POST":
        ...
        if status_code == 200:
            ...
            return redirect(
                url_for("analytics.view_analysis", analysis_id=result.get("id"))
            )
        else:
            if request.is_json:
                return jsonify(
                    {
                        "success": False,
                        "error": result.get("detail", "Failed to save analysis"),
                    }
                ), 400
...
def edit_analysis(analysis_id):
    ...
    if request.method == "POST":
        ...
        if request.form.get("columns"):
            try:
                ...
                flash("Invalid columns data format", "error")
                return redirect(
                    url_for("analytics.edit_analysis", analysis_id=analysis_id)
                )
        if request.form.get("chart_type"):
            data["chart_type"] = request.form.get("chart_type")

        result, status_code = api_request(
            "PUT", f"/analytics/analyses/{analysis_id}", data=data
        )
    ...
    sources, _ = api_request("GET", "/analytics/sources")
    fields, _ = api_request(
        "GET", f"/analytics/sources/{analysis.get('data_source', 'sales')}/fields"
    )
...
def toggle_favorite(analysis_id):
    ...
    result, status_code = api_request(
        "POST", f"/analytics/analyses/{analysis_id}/favorite"
    )

    if status_code == 200:
        return jsonify(
            {"success": True, "is_favorite": result.get("is_favorite", False)}
        )
    else:
        return jsonify(
            {
                "success": False,
                "error": result.get("detail", "Failed to toggle favorite"),
            }
        ), status_code
...
def new_dashboard():
    ...
    if request.method == "POST":
        ...
        if status_code == 200:
            ...
            return redirect(
                url_for("analytics.view_dashboard", dashboard_id=result.get("id"))
            )
        else:
...
def edit_dashboard(dashboard_id):
    ...
    if request.method == "POST":
        ...
        result, status_code = api_request(
            "PUT", f"/analytics/dashboards/{dashboard_id}", data=data
        )

        if status_code == 200:
            flash("Dashboard updated", "success")
            return redirect(
                url_for("analytics.view_dashboard", dashboard_id=dashboard_id)
            )
        else:
            flash(result.get("detail", "Failed to update"), "error")
    ...
    if dashboard.get("widgets"):
        dashboard["widgets"] = [
            {k: v for k, v in w.items() if k != "data"} for w in dashboard["widgets"]
        ]
...
def save_filter():
    ...
    try:
        ...
        return jsonify(
            {"success": False, "error": "Invalid filter configuration format"}
        ), 400
