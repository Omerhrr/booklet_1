...
def sales_report():
    ...
    report, status = api_request(
        "GET", "/reports/sales", params=params if params else None
    )
...
def sales_report_pdf():
    ...
    html = render_template(
        "reports/pdf/sales_report_pdf.html",
        report=report or {},
        business=business,
        start_date=start_date,
        end_date=end_date,
    ...
    response.headers["Content-Disposition"] = (
        f"attachment; filename=sales_report_{date.today()}.pdf"
    )
    return response
...
def sales_report_excel():
    ...
    response.headers["Content-Disposition"] = (
        f"attachment; filename=sales_report_{date.today()}.xlsx"
    )
    return response
...
def purchases_report():
    ...
    report, status = api_request(
        "GET", "/reports/purchases", params=params if params else None
    )
...
def purchases_report_pdf():
    ...
    report, _ = api_request(
        "GET", "/reports/purchases", params=params if params else None
    )
    business = get_business_info()

    html = render_template(
        "reports/pdf/purchases_report_pdf.html",
        report=report or {},
        business=business,
        start_date=start_date,
        end_date=end_date,
    ...
    response.headers["Content-Type"] = "application/pdf"
    response.headers["Content-Disposition"] = (
        f"attachment; filename=purchases_report_{date.today()}.pdf"
    )
    return response
...
def purchases_report_excel():
    ...
    response.headers["Content-Disposition"] = (
        f"attachment; filename=purchases_report_{date.today()}.xlsx"
    )
    return response
...
def expenses_report():
    ...
    report, status = api_request(
        "GET", "/reports/expenses", params=params if params else None
    )
...
def expenses_report_pdf():
    ...
    html = render_template(
        "reports/pdf/expenses_report_pdf.html",
        report=report or {},
        business=business,
        start_date=start_date,
        end_date=end_date,
    ...
    response.headers["Content-Type"] = "application/pdf"
    response.headers["Content-Disposition"] = (
        f"attachment; filename=expenses_report_{date.today()}.pdf"
    )
    return response
...
def expenses_report_excel():
    ...
    response.headers["Content-Disposition"] = (
        f"attachment; filename=expenses_report_{date.today()}.xlsx"
    )
    return response
...
def inventory_report():
    ...
    return render_template(
        "reports/inventory_report.html", title="Inventory Report", report=report
    )
...
def inventory_report_pdf():
    ...
    html = render_template(
        "reports/pdf/inventory_report_pdf.html",
        report=report or {},
        business=business,
        now=datetime.now(),
    )
    ...
    response.headers["Content-Type"] = "application/pdf"
    response.headers["Content-Disposition"] = (
        f"attachment; filename=inventory_report_{date.today()}.pdf"
    )
    return response
...
def aging_report():
    ...
    return render_template(
        "reports/aging_report.html", title="Aging Report", aging=aging
    )
...
def aging_report_pdf():
    ...
    html = render_template(
        "reports/pdf/aging_report_pdf.html",
        aging=aging or {},
        business=business,
        now=datetime.now(),
    )
    ...
    response.headers["Content-Type"] = "application/pdf"
    response.headers["Content-Disposition"] = (
        f"attachment; filename=aging_report_{date.today()}.pdf"
    )
    return response
...
def aging_report_excel():
    ...
    response.headers["Content-Disposition"] = (
        f"attachment; filename=aging_report_{date.today()}.xlsx"
    )
    return response
...
def trial_balance():
    ...
    report, status = api_request(
        "GET", "/reports/trial-balance", params=params if params else None
    )
...
def trial_balance_pdf():
    ...
    response.headers["Content-Type"] = "application/pdf"
    response.headers["Content-Disposition"] = (
        f"attachment; filename=trial_balance_{date.today()}.pdf"
    )
    return response
...
def trial_balance_excel():
    ...
    for row, acc in enumerate(report.get("accounts", []), 5):
        ...
        debit_cell = ws.cell(
            row=row, column=3, value=acc.get("debit") if acc.get("debit") else None
        )
        debit_cell.number_format = currency_format
        debit_cell.border = thin_border
        credit_cell = ws.cell(
            row=row, column=4, value=acc.get("credit") if acc.get("credit") else None
        )
    ...
    ws.cell(
        row=total_row, column=3, value=report.get("total_debit")
    ).number_format = currency_format
    ws.cell(row=total_row, column=3).font = Font(bold=True)
    ws.cell(
        row=total_row, column=4, value=report.get("total_credit")
    ).number_format = currency_format
    ...
    response.headers["Content-Disposition"] = (
        f"attachment; filename=trial_balance_{date.today()}.xlsx"
    )
    return response
...
def vat_report():
    ...
    report, status = api_request(
        "GET", "/reports/vat", params=params if params else None
    )
...
def vat_report_pdf():
    ...
    html = render_template(
        "reports/pdf/vat_report_pdf.html",
        report=report or {},
        business=business,
        start_date=start_date,
        end_date=end_date,
    ...
    response.headers["Content-Type"] = "application/pdf"
    response.headers["Content-Disposition"] = (
        f"attachment; filename=vat_report_{date.today()}.pdf"
    )
    return response
...
def vat_report_excel():
    ...
    response.headers["Content-Disposition"] = (
        f"attachment; filename=vat_report_{date.today()}.xlsx"
    )
    return response
