...
def list_categories():
    ...
    return render_template(
        "inventory/categories.html", title="Categories", categories=categories
    )
...
def create_category():
    ...
    if status == 200:
        if request.headers.get("HX-Request"):
            return render_template(
                "inventory/partials/category_row.html", category=response
            )
        flash("Category created", "success")
        return redirect(url_for("inventory.list_categories"))

    error = (
        response.get("detail", "Failed to create category") if response else "Failed"
    )
...
def edit_category(category_id):
    ...
    if request.method == "GET":
        ...
        return render_template(
            "inventory/partials/category_row_edit.html", category=category
        )
    ...
    response, status = api_request(
        "PUT", f"/inventory/categories/{category_id}", data=data
    )
...
def list_products():
    ...
    products, status = api_request(
        "GET",
        f"/inventory/products?include_inactive=true&page={page}&per_page={per_page}",
    )
...
def low_stock():
    ...
    return render_template(
        "inventory/low_stock.html", title="Low Stock", products=products
    )
...
def edit_product(product_id):
    ...
    response, status = api_request(
        "PUT", f"/inventory/products/{product_id}", data=data
    )
    ...
    error = (
        response.get("detail", "Failed to update product")
        if response
        else "Failed to update product"
    )
    flash(error, "error")
    categories, _ = api_request("GET", "/inventory/categories")
...
def toggle_product_status(product_id):
    ...
    response, status = api_request(
        "POST", f"/inventory/products/{product_id}/toggle-status"
    )
...
def adjust_stock(product_id):
    ...
    response, status = api_request(
        "POST", f"/inventory/products/{product_id}/adjust-stock", data=data
    )

    if status == 200:
        if request.headers.get("HX-Request"):
            return render_template(
                "inventory/partials/product_row.html", product=response
            )
...
def stock_adjustments():
    ...
    return render_template(
        "inventory/stock_adjustments.html",
