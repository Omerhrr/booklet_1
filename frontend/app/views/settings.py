...
def check_plan_limit(resource_type):
    """Check if the business has reached its plan limit for a resource type.

    Args:
        resource_type: 'branches' or 'users'

    Returns:
        tuple: (can_create: bool, current_count: int, max_limit: int)
    """
    plan_limits = session.get(
        "plan_limits", {"max_branches": 1, "max_users": 5, "slug": "basic"}
    )

    if resource_type == "branches":
        max_limit = plan_limits.get("max_branches", 1)
        branches = session.get("branches", [])
        current_count = len(branches)
    elif resource_type == "users":
        max_limit = plan_limits.get("max_users", 5)
        users_data, _ = api_request("GET", "/settings/users")
        current_count = len(users_data) if isinstance(users_data, list) else 0
    else:
        return True, 0, 0
...
def business_settings():
    """Business settings - redirect to settings page with business tab"""
...
def new_branch():
    """Create new branch"""
    # Check plan limits
    can_create, current_count, max_limit = check_plan_limit("branches")

    if not can_create:
        flash(
            f"Your plan allows a maximum of {max_limit} branches. You currently have {current_count}. Please upgrade your plan.",
            "error",
        )
        return redirect(url_for("settings.index"))
    ...
    if status == 200:
        ...
        error_msg = (
            response.get("detail", "Failed to create branch")
            if response
            else "Failed to create branch"
        )
        # Check if it's a plan limit error
        if status == 403 and "plan" in error_msg.lower():
            flash(error_msg + " Please upgrade your subscription.", "error")
        else:
...
def edit_branch(branch_id):
    ...
    if request.method == "GET":
        ...
        return render_template(
            "settings/branch_form.html", title="Edit Branch", branch=branch
        )
...
def set_default_branch(branch_id):
    ...
    response, status = api_request(
        "POST", f"/settings/branches/{branch_id}/set-default"
    )
...
def new_role():
    ...
    if request.method == "GET":
        ...
        return render_template(
            "settings/role_form.html", title="New Role", permissions=permissions or []
        )
...
def delete_role(role_id):
    ...
    if status == 200:
        ...
        error_msg = (
            response.get("detail", "Failed to delete role")
            if response
            else "Failed to delete role"
        )
        flash(error_msg, "error")
...
def view_user(user_id):
    ...
    return render_template(
        "settings/user_detail.html",
        title=f"{user.get('username', 'User')} - Details",
        user=user,
    )
...
def new_user():
    """Create new user"""
    # Check plan limits (approximate - backend will enforce exact limit)
    plan_limits = session.get("plan_limits", {"max_users": 5})
    max_users = plan_limits.get("max_users", 5)
    ...
    if status == 200:
        ...
        if role_id and branch_id:
            ...
            assign_response, assign_status = api_request(
                "POST", "/settings/users/assign-role", data=assign_data
            )
        else:
            pass
        ...
        error = (
            response.get("detail", "Failed to create user") if response else "Failed"
        )
        # Check if it's a plan limit error
        if status == 403 and "plan" in error.lower():
            flash(error + " Please upgrade your subscription.", "error")
        else:
            flash(error, "error")
...
def delete_user(user_id):
    ...
    if status == 200:
        ...
        error_msg = (
            response.get("detail", "Failed to delete user")
            if response
...
def seed_permissions():
    ...
    if status == 200:
        flash(
            "Permissions synced successfully. Admin role updated with all permissions.",
            "success",
        )
    else:
        error_msg = (
            response.get("detail", "Failed to seed permissions")
            if response
            else "Failed to seed permissions"
        )
        flash(error_msg, "error")
...
def new_fiscal_year():
    ...
    if request.method == "GET":
        return render_template(
            "settings/fiscal_year_form.html", title="New Fiscal Year"
        )
    ...
    if status == 200:
        ...
        error_msg = (
            response.get("detail", "Failed to create fiscal year")
            if response
            else "Failed to create fiscal year"
        )
        flash(error_msg, "error")
...
def set_current_fiscal_year(fy_id):
    ...
    if status == 200:
        ...
        error_msg = (
            response.get("detail", "Failed to set fiscal year as current")
            if response
            else "Failed to set fiscal year as current"
        )
        flash(error_msg, "error")
...
def close_fiscal_year(fy_id):
    ...
    if status == 200:
        ...
        error_msg = (
            response.get("detail", "Failed to close fiscal year")
            if response
            else "Failed to close fiscal year"
        )
        flash(error_msg, "error")
