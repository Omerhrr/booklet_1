...
def check_plan_limit(resource_type):
    ...
    if resource_type == 'branches':
        ...
        # Users count should come from API, approximate for now
        current_count = 0  # Will be checked on backend
    else:
        return True, 0, 0
...
def new_branch():
    ...
    if not can_create:
        flash(f'Your plan allows a maximum of {max_limit} branches. You currently have {current_count}. Please upgrade your plan.', 'error')
        return redirect(url_for('settings.index'))
...
def new_user():
    ...
    if request.method == 'GET':
        ...
        return render_template('settings/user_form.html', title='New User', roles=roles_data or [], branches=branches_data or [], max_users=max_users)
