...
def view_account(account_id):
    ...
    # Get balance
    balance_data, balance_status = api_request(
        "GET", f"/accounting/accounts/{account_id}/balance"
    )
    ...
    ledger_data, ledger_status = api_request(
        "GET", f"/accounting/accounts/{account_id}/ledger"
    )
    ledger_entries = (
        ledger_data.get("entries", []) if ledger_status == 200 and ledger_data else []
    )

    return render_template(
        "accounting/account_detail.html",
        title=account_info.get("name", "Account")
        if isinstance(account_info, dict)
        else getattr(account_info, "name", "Account"),
...
def journal():
    ...
    return render_template(
        "accounting/journal_list.html", title="Journal Entries", journals=journals
    )
...
def view_journal(journal_id):
    ...
    return render_template(
        ...
        title=f"Journal {journal.get('voucher_number', '')}",
        journal=journal,
    )
...
def new_journal():
    ...
    try:
        ...
        flash("Invalid journal entry lines data", "error")
    ...
    error = (
        response.get("detail", "Failed to create journal entry")
        if response
        else "Failed"
    )
    flash(error, "error")
...
def post_journal(journal_id):
    ...
    response, status = api_request(
        "POST", f"/accounting/journal-vouchers/{journal_id}/post"
    )
    ...
    if status == 200:
        ...
        error = (
            response.get("detail", "Failed to post journal entry")
            if response
            else "Failed"
        )
        flash(error, "error")
...
def general_ledger():
    ...
    if status == 200 and ledger_data:
        ...
        summary = (
            ledger_data.get("summary", {}) if isinstance(ledger_data, dict) else {}
        )
...
def general_ledger_pdf():
    ...
    html = render_template(
        "accounting/pdf/general_ledger_pdf.html",
        entries=entries,
        summary=summary,
        business=business,
    ...
    response.headers["Content-Disposition"] = (
        f"attachment; filename=general_ledger_{start_date or 'all'}_to_{end_date or 'now'}.pdf"
    )
    return response
...
def general_ledger_excel():
    ...
    debit_fill = PatternFill(
        start_color="C6EFCE", end_color="C6EFCE", fill_type="solid"
    )
    credit_fill = PatternFill(
    ...
    if summary and summary.get("accounts_summary"):
        ...
        for acc in summary.get("accounts_summary", []):
            ws.cell(
                row=row, column=1, value=acc.get("account_code")
            ).border = thin_border
            ws.cell(
                row=row, column=2, value=acc.get("account_name")
            ).border = thin_border
            ws.cell(
                row=row, column=3, value=acc.get("account_type")
            ).border = thin_border
            ws.cell(
                row=row, column=4, value=acc.get("opening_balance")
            ).number_format = currency_format
            ws.cell(row=row, column=4).border = thin_border
            ws.cell(
                row=row, column=5, value=acc.get("total_debit")
            ).number_format = currency_format
            ws.cell(row=row, column=5).border = thin_border
            ws.cell(
                row=row, column=6, value=acc.get("total_credit")
            ).number_format = currency_format
            ws.cell(row=row, column=6).border = thin_border
            ws.cell(
                row=row, column=7, value=acc.get("closing_balance")
            ).number_format = currency_format
    ...
    for entry in entries:
        ...
        account_cell = ws.cell(
            row=row,
            column=2,
            value=f"{entry.get('account_code', '')} {entry.get('account_name', '')}",
        )
    ...
    ws.cell(
        row=row, column=5, value=summary.get("total_debit")
    ).number_format = currency_format
    ws.cell(row=row, column=5).font = Font(bold=True)
    ws.cell(row=row, column=5).fill = total_fill
    ws.cell(
        row=row, column=6, value=summary.get("total_credit")
    ).number_format = currency_format
    ws.cell(row=row, column=6).font = Font(bold=True)
    ws.cell(row=row, column=6).fill = total_fill
    ws.cell(
        row=row, column=7, value=summary.get("net_movement")
    ).number_format = currency_format
    ...
    response.headers["Content-Disposition"] = (
        f"attachment; filename=general_ledger_{start_date or 'all'}_to_{end_date or 'now'}.xlsx"
    )
    return response
...
def trial_balance():
    ...
    if as_of_date:
        data, status = api_request(
            "GET", f"/accounting/reports/trial-balance?as_of_date={as_of_date}"
        )
...
def trial_balance_pdf():
    ...
    html = render_template(
        "reports/pdf/trial_balance_pdf.html",
        report=report,
        business=business,
        as_of_date=as_of_date,
        now=datetime.now(),
    ...
    response.headers["Content-Type"] = "application/pdf"
    response.headers["Content-Disposition"] = (
        f"attachment; filename=trial_balance_{as_of_date or date.today()}.pdf"
    )
    return response
...
def balance_sheet_pdf():
    ...
    if as_of_date:
        data, status = api_request(
            "GET", f"/accounting/reports/balance-sheet?as_of_date={as_of_date}"
        )
        if status == 200 and data:
            report = data
    ...
    html = render_template(
        "reports/pdf/balance_sheet_pdf.html",
        report=report,
        business=business,
        now=datetime.now(),
    )
    ...
    response.headers["Content-Type"] = "application/pdf"
    response.headers["Content-Disposition"] = (
        f"attachment; filename=statement_of_financial_position_{as_of_date or date.today()}.pdf"
    )
    return response
...
def balance_sheet_excel():
    ...
    ws.cell(
        row=row, column=2, value=report["non_current_assets"]["fixed_assets_cost"]
    ).number_format = currency_format
    ws.cell(row=row, column=2).border = thin_border
    row += 1

    ws.cell(
        row=row, column=1, value="Less: Accumulated Depreciation"
    ).border = thin_border
    ws.cell(
        row=row,
        column=2,
        value=abs(report["non_current_assets"]["accumulated_depreciation"]),
    ).number_format = currency_format
    ws.cell(row=row, column=2).border = thin_border
    row += 1
    ...
    ws.cell(
        row=row, column=2, value=report["non_current_assets"]["net_book_value"]
    ).number_format = currency_format
    ...
    if report["non_current_assets"]["other_non_current"] != 0:
        ws.cell(
            row=row, column=1, value="Other Non-Current Assets"
        ).border = thin_border
        ws.cell(
            row=row, column=2, value=report["non_current_assets"]["other_non_current"]
        ).number_format = currency_format
        ws.cell(row=row, column=2).border = thin_border
        row += 1
    ...
    ws.cell(
        row=row, column=2, value=report["non_current_assets"]["total"]
    ).number_format = currency_format
    ws.cell(row=row, column=2).font = Font(bold=True)
    ...
    if report["current_assets"]["vendor_advances"] != 0:
        current_asset_items.append(
            ("Vendor Advances", report["current_assets"]["vendor_advances"])
        )
    if report["current_assets"]["other_current_assets"] != 0:
        current_asset_items.append(
            ("Other Current Assets", report["current_assets"]["other_current_assets"])
        )
    ...
    ws.cell(row=row, column=1).fill = subtotal_fill
    ws.cell(
        row=row, column=2, value=report["current_assets"]["total"]
    ).number_format = currency_format
    ws.cell(row=row, column=2).font = Font(bold=True)
    ws.cell(row=row, column=2).fill = subtotal_fill
    row += 2
    ...
    ws.cell(row=row, column=1).fill = asset_fill
    ws.cell(
        row=row, column=2, value=report["total_assets"]
    ).number_format = currency_format
    ...
    if report["liabilities"]["customer_advances"] != 0:
        liability_items.append(
            ("Customer Advances", report["liabilities"]["customer_advances"])
        )
    if report["liabilities"]["other_liabilities"] != 0:
        liability_items.append(
            ("Other Liabilities", report["liabilities"]["other_liabilities"])
        )
    ...
    ws.cell(row=row, column=1).fill = subtotal_fill
    ws.cell(
        row=row, column=2, value=report["liabilities"]["total"]
    ).number_format = currency_format
    ws.cell(row=row, column=2).font = Font(bold=True)
    ws.cell(row=row, column=2).fill = subtotal_fill
    row += 2
    ...
    equity_items = [
        ...
        (
            ...
            report["equity"]["current_period_earnings"],
        ),
    ]

    if report["equity"]["retained_earnings"] != 0:
        equity_items.append(
            ("Retained Earnings (Prior Periods)", report["equity"]["retained_earnings"])
        )
    if report["equity"]["opening_balance_equity"] != 0:
        equity_items.append(
            ("Opening Balance Equity", report["equity"]["opening_balance_equity"])
        )
    ...
    ws.cell(row=row, column=1).fill = subtotal_fill
    ws.cell(
        row=row, column=2, value=report["equity"]["total"]
    ).number_format = currency_format
    ws.cell(row=row, column=2).font = Font(bold=True)
    ws.cell(row=row, column=2).fill = subtotal_fill
    row += 2
    ...
    ws.cell(row=row, column=1).fill = equity_fill
    ws.cell(
        row=row, column=2, value=report["total_equity_and_liabilities"]
    ).number_format = currency_format
    ws.cell(row=row, column=2).font = header_font
    ws.cell(row=row, column=2).fill = equity_fill
    row += 2
    ...
    if diff > 0.01:
        ws.cell(
            row=row, column=1, value=f"Warning: Balance Sheet difference of {diff:.2f}"
        )
    ...
    response.headers["Content-Disposition"] = (
        f"attachment; filename=statement_of_financial_position_{as_of_date or date.today()}.xlsx"
    )
    return response
...
def profit_loss_pdf():
    ...
    if start_date and end_date:
        data, status = api_request(
            "GET",
            f"/accounting/reports/income-statement?start_date={start_date}&end_date={end_date}",
        )
        if status == 200 and data:
            report = data
    ...
    response.headers["Content-Type"] = "application/pdf"
    response.headers["Content-Disposition"] = (
        f"attachment; filename=profit_loss_{end_date or date.today()}.pdf"
    )
    return response
...
def profit_loss_excel():
    ...
    ws.cell(
        row=row, column=3, value=report.get("total_revenue", 0)
    ).number_format = currency_format
    ...
    total_cell.fill = total_fill
    ws.cell(
        row=row, column=3, value=report.get("total_expenses", 0)
    ).number_format = currency_format
    ws.cell(row=row, column=3).font = Font(bold=True)
    ws.cell(row=row, column=3).fill = total_fill
    row += 2
    ...
    ws.cell(row=row, column=2, value="NET INCOME (PROFIT/LOSS)").font = Font(
        bold=True, size=12
    )
    ...
    response.headers["Content-Disposition"] = (
        f"attachment; filename=profit_loss_{end_date or date.today()}.xlsx"
    )
    return response
