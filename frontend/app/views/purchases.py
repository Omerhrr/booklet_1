...
def list_bills():
    ...
    total = len(bills) if isinstance(bills, list) else 0
    pages = max(1, -(-total // per_page))

    return render_template(
        "purchases/bills.html",
...
def record_payment(bill_id):
    ...
    response, status = api_request(
        "POST", f"/purchases/bills/{bill_id}/payment", data=data
    )
...
def list_debit_notes():
    ...
    return render_template(
        "purchases/debit_notes.html", title="Debit Notes", debit_notes=debit_notes
    )
...
def view_debit_note(debit_note_id):
    ...
    return render_template(
        "purchases/debit_note_detail.html",
...
def create_debit_note(bill_id):
    ...
    if request.method == "GET":
        return render_template(
            "purchases/debit_note_form.html",
    ...
    if status == 200:
        ...
        return redirect(
            url_for("purchases.view_debit_note", debit_note_id=response.get("id"))
        )

    error = (
        response.get("detail", "Failed to create debit note")
        if response
        else "Failed to create debit note"
    )
    flash(error, "error")
    return redirect(url_for("purchases.create_debit_note", bill_id=bill_id))
...
def apply_debit_note(debit_note_id):
    ...
    response, status = api_request(
        "POST", f"/purchases/debit-notes/{debit_note_id}/apply"
    )

    if status == 200:
        flash("Debit note applied to bill successfully", "success")
        return redirect(
            url_for("purchases.view_debit_note", debit_note_id=debit_note_id)
        )

    error = (
        response.get("detail", "Failed to apply debit note") if response else "Failed"
    )
    flash(error, "error")
