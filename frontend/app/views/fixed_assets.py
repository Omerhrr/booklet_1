...
@login_required
def check_fixed_assets_feature():
    """Check if Fixed Assets feature is available for the current plan"""
...
def create_fixed_asset():
    ...
    if request.method == 'GET':
        ...
        liability_accounts = [a for a in (accounts or []) if a.get('type') == 'Liability']
        
        # Cash/bank accounts for fund source (also include liability for accounts payable)
        cash_bank_accounts = [a for a in (accounts or []) if 
                            a.get('type') == 'Asset' and 
                            any(keyword in a.get('name', '').lower() for keyword in ['cash', 'bank'])]
        
        # Combine cash/bank and liability (accounts payable) for fund source options
        fund_source_accounts = cash_bank_accounts + liability_accounts
        ...
        return render_template('fixed_assets/form.html', 
                              ...
                              fund_source_accounts=fund_source_accounts,
                              today=date.today().isoformat())
    
    # POST - Create fixed asset
    is_existing_asset = request.form.get('is_existing_asset') == 'true'
    ...
    data = {
        ...
        'existing_accumulated_depreciation': float(request.form.get('existing_accumulated_depreciation', 0)) if is_existing_asset else 0,
        # Fund source for new assets
        'fund_source_account_id': int(request.form.get('fund_source_account_id')) if request.form.get('fund_source_account_id') and not is_existing_asset else None,
    }
