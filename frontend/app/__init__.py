    ...
    try:
        ...
        if response.content:
            ...
            if "application/json" in content_type:
                ...
                return {
                    "error": f"Server returned non-JSON response (status {response.status_code})",
...
def login_required(f):
    """Decorator to require login with periodic token expiration check"""
    ...
    def decorated_function(*args, **kwargs):
        ...
        last_check = session.get("_last_token_check", 0)
        now = time.time()
        if now - last_check > 300:
            response, status = api_request("GET", "/auth/me")
            session["_last_token_check"] = now
            if status == 401:
                session.clear()
                return redirect(url_for("auth.login"))
...
def inject_globals():
    ...
    plan_limits = session.get(
        "plan_limits",
        {"max_branches": 1, "max_users": 5, "includes_agents": False, "slug": "basic"},
    )
    ...
    def can_create_user():
        ...
        plan_limits = session.get("plan_limits", {"max_users": 5, "slug": "basic"})
    ...
    def has_permission_or_superuser(perm):
        ...
        """
        return perm in session.get("permissions", []) or session.get(
            "is_superuser", False
        )
...
def currency_filter(value, symbol=None):
    ...
    if symbol is None:
        ...
        elif len(currency_code) == 1 or currency_code not in [
            "USD",
            "NGN",
            "EUR",
            "GBP",
